...
export default function AssessmentForm({
  ...
  const filteredCourses = form.courseSearch
    ? courseOptions.filter(c =>
      c.title.toLowerCase().includes(form.courseSearch.toLowerCase())
    )
    : courseOptions
            ...
            {isFinalExam ? (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    updateField('courseSearch', e.target.value)
                    if (e.target.value !== form.courseLabel) {
                      updateField('course', '')
                      updateField('courseLabel', '')
                    }
                  }}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-blue-500 dark:focus:ring-[#201a7c]/20 focus:border-blue-500 dark:focus:border-[#201a7c] text-gray-900 dark:text-gray-100 bg-white dark:bg-[var(--card-background)]"
                ...
                {form.courseSearch !== form.courseLabel && filteredCourses.length > 0 ? (
                  <div className="mt-1 border border-gray-200 dark:border-[var(--card-border)] rounded-lg max-h-40 overflow-y-auto bg-white dark:bg-[var(--card-background)] shadow-sm">
                    {filteredCourses.map((c) => (
                      <button
                        key={c.id}
                        onClick={() => {
                          updateField('course', c.id)
                          updateField('courseSearch', c.title)
                          updateField('courseLabel', c.title)
                        }}
                  ...
                  <div className="mt-1 border border-gray-200 dark:border-[var(--card-border)] rounded-lg max-h-40 overflow-y-auto bg-white dark:bg-[var(--card-background)] shadow-sm">
                    ...
                    {courseOptions.length === 0 && (
                      <p className="px-3 py-2 text-sm text-gray-400 dark:text-gray-500">No courses available</p>
                    )}
                  </div>
                ) : null}
              </div>
          ...
          <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] shadow-sm p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={10}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-blue-500 dark:focus:ring-[#201a7c]/20 focus:border-blue-500 dark:focus:border-[#201a7c] text-gray-900 dark:text-gray-100 bg-white dark:bg-[var(--card-background)]"
                placeholder="Assessment description... (supports JSON/Lexical format)"
              />
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {items.length > 1 && (
                <div className="flex gap-2 text-xs text-gray-500 dark:text-gray-400">
                  <button
                    onClick={() => setItems(prev => prev.map((i, idx) => ({ ...i, order: idx + 1 })).sort((a, b) => a.order - b.order))}
                      ...
                      <div>
                        ...
                        {item.questionSearch !== item.questionLabel && fq.length > 0 ? (
                          ...
                          <div className="mt-1 border border-gray-200 dark:border-[var(--card-border)] rounded-lg max-h-40 overflow-y-auto bg-white dark:bg-[var(--card-background)] shadow-sm">
                            ...
                            {questions.length === 0 && (
                              <p className="px-3 py-2 text-sm text-gray-400 dark:text-gray-500">No questions available</p>
                            )}
                          </div>
                        ) : null}
                      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] shadow-sm p-6 space-y-3">
            ...
            {mode === 'create' && (
              <div>
                ...
                <p className="text-sm text-gray-900 dark:text-gray-100 font-medium">
                  {TYPE_LABELS[form.assessmentType]}
                </p>
              </div>
            )}
            {isFinalExam ? (
            ...
            <div>
              ...
              <p className="text-sm text-gray-900 dark:text-gray-100 font-medium mt-1">
                {form.timeLimitMinutes > 0 ? `${form.timeLimitMinutes} min` : 'No limit'}
              </p>
            </div>
          </div>
