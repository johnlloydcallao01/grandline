...
export default function InstructorSearchResultsPage() {
      ...
      <div className="md:px-[10px] px-[10px] md:pt-2 pt-2 md:pb-4 pb-4 lg:hidden sticky top-0 z-30 bg-[var(--background)]">
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Back"
          >
            <i className="fa fa-arrow-left text-gray-700 dark:text-gray-300" />
          </button>
          <div className="flex-1 flex items-center bg-[var(--card-background)] rounded-full shadow-sm border border-[var(--card-border)] px-3 py-1.5">
            <input
              ...
              value={inputValue}
              readOnly
              className="flex-1 bg-transparent border-0 outline-none text-sm text-gray-900 dark:text-gray-100"
              placeholder="Search"
            />
          </div>
        </div>
          ...
          {isLoading ? (
              ...
              {Array.from({ length: 6 }).map((_, idx) => (
                <div key={idx} className="p-4">
                  <div className="flex items-center gap-4 animate-pulse">
                    <div className="w-10 h-10 rounded-md bg-gray-200 dark:bg-gray-800 flex-shrink-0" />
            ...
            <div className="p-10 text-center">
              ...
              <div className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Try different keywords within your courses.
              </div>
            </div>
          ) : results.length > 0 ? (
            <div className="divide-y divide-[var(--card-border)]">
              {results.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <i className="fa fa-chevron-right text-gray-400 dark:text-gray-500 flex-shrink-0" />
                  </Link>
                )
              })}
            </div>
