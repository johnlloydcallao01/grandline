...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    CREATE INDEX IF NOT EXISTS "acct_currencies_name_idx" ON "acct_currencies" USING btree ("name");
    CREATE INDEX IF NOT EXISTS "acct_currencies_is_active_idx" ON "acct_currencies" USING btree ("is_active");
    CREATE INDEX IF NOT EXISTS "acct_payment_terms_name_idx" ON "acct_payment_terms" USING btree ("name");
    CREATE INDEX IF NOT EXISTS "acct_payment_terms_is_active_idx" ON "acct_payment_terms" USING btree ("is_active");
    CREATE INDEX IF NOT EXISTS "accounting_customers_currency_reference_idx" ON "accounting_customers" USING btree ("currency_reference_id");
    CREATE INDEX IF NOT EXISTS "accounting_customers_payment_term_reference_idx" ON "accounting_customers" USING btree ("payment_term_reference_id");
    ...
    BEGIN
      IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'accounting_customers_currency_reference_id_acct_currencies_id_fk'
      ) THEN
        ALTER TABLE "accounting_customers"
          ADD CONSTRAINT "accounting_customers_currency_reference_id_acct_currencies_id_fk"
    ...
    BEGIN
      IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'accounting_customers_payment_term_reference_id_acct_payment_terms_id_fk'
      ) THEN
        ALTER TABLE "accounting_customers"
          ADD CONSTRAINT "accounting_customers_payment_term_reference_id_acct_payment_terms_id_fk"
    ...
    SELECT DISTINCT
      ...
      0,
      'Backfilled from existing accounting customer payment terms.',
      true
    FROM "accounting_customers"
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "accounting_customers"
      DROP CONSTRAINT IF EXISTS "accounting_customers_currency_reference_id_acct_currencies_id_fk";
    ALTER TABLE "accounting_customers"
      DROP CONSTRAINT IF EXISTS "accounting_customers_payment_term_reference_id_acct_payment_terms_id_fk";

    DROP INDEX IF EXISTS "accounting_customers_currency_reference_idx";
    DROP INDEX IF EXISTS "accounting_customers_payment_term_reference_idx";
    DROP INDEX IF EXISTS "acct_currencies_name_idx";
    DROP INDEX IF EXISTS "acct_currencies_is_active_idx";
    DROP INDEX IF EXISTS "acct_payment_terms_name_idx";
    DROP INDEX IF EXISTS "acct_payment_terms_is_active_idx";

    ALTER TABLE "accounting_customers"
      DROP COLUMN IF EXISTS "currency_reference_id",
      DROP COLUMN IF EXISTS "payment_term_reference_id";

    DROP TABLE IF EXISTS "acct_payment_terms";
    DROP TABLE IF EXISTS "acct_currencies";
  `)
}
