...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_course_feedbacks_fk";
  
  DROP INDEX IF EXISTS "payload_locked_documents_rels_course_feedbacks_id_idx";
  ALTER TABLE "payload_locked_documents_rels" DROP COLUMN IF EXISTS "course_feedbacks_id";`)
}
