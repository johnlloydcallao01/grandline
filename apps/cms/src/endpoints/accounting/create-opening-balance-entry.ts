...
export const createAccountingOpeningBalanceEntryEndpoint = async (req: PayloadRequest) => {
  try {
    const user = requireAccountingAdmin(req)
    const body = await parseAccountingRequestBody(req)

    if (!Array.isArray(body?.lines) || body.lines.length === 0) {
      throw new APIError('lines are required to create an opening balance entry.', 400)
    }
    ...
    console.error('Error creating accounting opening balance entry:', error)
