...
import { ACCOUNTING_COLLECTION_SLUGS, ACCOUNTING_FIXED_ASSET_CATEGORY_OPTIONS, ACCOUNTING_FIXED_ASSET_STATUS_OPTIONS, ACCOUNTING_DEPRECIATION_METHOD_OPTIONS } from '@/accounting/constants/accounting'
...
const methodMap = new Map(ACCOUNTING_DEPRECIATION_METHOD_OPTIONS.map((o) => [o.value, o.label]))
...
const lookupLabel = (doc: unknown): { id: string; label: string } | null => {
  const d = doc as Record<string, unknown> | undefined
  if (!d) return null
  const id = String(d.id ?? '')
  const code = typeof d.code === 'string' ? d.code : null
  const name = typeof d.name === 'string' ? d.name : null
  return { id, label: [code, code && name ? '-' : null, name].filter(Boolean).join(' ') || id }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = result.docs.map((doc) => {
      ...
      const sv = Number(d.salvageValue || 0)
      ...
      const methodLbl = methodMap.get(String(d.depreciationMethod || '') as typeof ACCOUNTING_DEPRECIATION_METHOD_OPTIONS[number]['value']) || '-'
      const statusTone = st === 'active' ? 'green' : st === 'fully_depreciated' ? 'blue' : st === 'disposed' ? 'amber' : st === 'written_off' ? 'red' : 'gray'

      const assetAcct = lookupLabel(d.assetAccount)
      const expenseAcct = lookupLabel(d.expenseAccount)
      const accumAcct = lookupLabel(d.accumulatedDepreciationAccount)
      ...
      return {
        ...
        costLabel: fmt(cost),
        salvageValue: sv,
        salvageValueLabel: fmt(sv),
        usefulLifeMonths: ulm,
        usefulLifeLabel: `${ulm} months`,
        depreciationMethod: String(d.depreciationMethod || ''),
        methodLabel: methodLbl,
        expenseAccountId: expenseAcct?.id || null,
        expenseAccountLabel: expenseAcct?.label || '-',
        assetAccountId: assetAcct?.id || null,
        assetAccountLabel: assetAcct?.label || '-',
        accumulatedDepreciationAccountId: accumAcct?.id || null,
        accumulatedDepreciationAccountLabel: accumAcct?.label || '-',
        status: st,
        statusLabel: stLbl,
        statusTone,
        notes: String(d.notes || ''),
        cells: [
          { text: String(d.assetCode || '-'), emphasis: true },
          String(d.name || '-'),
          catLbl,
        ...
        setupCells: [
          { text: String(d.name || '-'), emphasis: true },
          catLbl,
          methodLbl,
          `${ulm} months`,
          { text: fmt(sv), align: 'right' },
          [assetAcct?.label, accumAcct?.label, expenseAcct?.label].filter(Boolean).join(' / ') || '-',
        ],
      }
    })
    ...
    const all = allDocs.docs.map((d) => d as unknown as Record<string, unknown>)
    const allCost = all.reduce((s, d) => s + Number(d.cost || 0), 0)
    const activeCount = all.filter((d) => String(d.status || '') === 'active').length
    const draftCount = all.filter((d) => String(d.status || '') === 'draft').length
    ...
    return NextResponse.json({
      rows,
      metrics: [
        { id: 'total-assets', label: 'Total Assets', value: all.length, change: 'Assets in register', trend: 'up' as const },
        { id: 'active-assets', label: 'Active Assets', value: activeCount, change: 'Currently in service', trend: activeCount > 0 ? 'up' as const : 'neutral' as const },
        { id: 'draft-assets', label: 'Draft Assets', value: draftCount, change: 'Pending activation', trend: 'neutral' as const },
        { id: 'register-cost', label: 'Register Cost', value: fmt(allCost), change: 'Gross acquisition cost', trend: 'up' as const },
      ],
      filterOptions: {
      ...
      meta: {
        ...
        setupColumns: ['Asset Name', 'Category', 'Method', 'Useful Life', 'Salvage Value', 'Account Mapping'],
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data: Record<string, unknown> = {
      ...
      expenseAccount: body.expenseAccount || null,
      assetAccount: body.assetAccount || null,
      accumulatedDepreciationAccount: body.accumulatedDepreciationAccount || null,
