...
import {
  buildAgingInvoiceRow,
  buildAgingMetrics,
  buildAgingReferenceData,
  isOpenReceivableAgingInvoice,
  matchesSelectedAgingFilters,
  normalizeSearch,
  parseIntegerParam,
  parseListParam,
  type AgingInvoiceDoc,
} from './_shared'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [invoiceDocs, referenceData] = await Promise.all([
      findAllDocs<AgingInvoiceDoc>({
        payload,
        collection: ACCOUNTING_COLLECTION_SLUGS.invoices,
        depth: 1,
        sort: 'dueDate',
      }),
      buildAgingReferenceData(payload),
    ])

    const allRows = invoiceDocs
      .filter(isOpenReceivableAgingInvoice)
      .map(buildAgingInvoiceRow)
      .sort((left, right) => {
    ...
    const filteredRows = allRows.filter((row) => {
      ...
      return matchesSelectedAgingFilters(row, {
        statuses,
        customerIds,
        agingBuckets,
        quickFilters,
    ...
    return NextResponse.json({
      rows: paginatedRows,
      metrics: buildAgingMetrics(filteredRows),
      filterOptions: {
