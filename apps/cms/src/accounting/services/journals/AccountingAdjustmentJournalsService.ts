...
export type AdjustmentJournalsQuery = {
  search?: string
  statuses?: string[]
...
export type AdjustmentJournalsRow = {
  id: number | string
  entryNumber: string | null
  postingDate: string | null
  referenceNumber: string | null
...
export type AdjustmentJournalsResult = {
  rows: AdjustmentJournalsRow[]
  metrics: Metric[]
...
const mapRow = (doc: AdjustmentDoc): AdjustmentJournalsRow => {
  ...
  return {
    ...
    referenceNumber: doc.referenceNumber || null, memo: doc.memo || null,
...
const matchesSearch = (row: AdjustmentJournalsRow, search: string) => {
  if (!search) return true
  return [row.entryNumber, row.referenceNumber, row.memo, row.statusLabel].some((v) => normalizeSearch(v).includes(search))
}

const matchesStatuses = (row: AdjustmentJournalsRow, statuses: string[]) => {
  if (!statuses.length) return true
  return Boolean(row.status && statuses.includes(row.status))
}

const matchesBalanced = (row: AdjustmentJournalsRow, balancedFilters: string[]) => {
...
const sortEntries = (docs: AdjustmentDoc[]) => [...docs].sort((a, b) => String(b.entryDate || '').localeCompare(String(a.entryDate || '')))

const buildMetrics = (rows: AdjustmentJournalsRow[]): Metric[] => {
  ...
  return [
    { id: 'adjustment-entries', label: 'Adjustment Entries', value: total, change: 'Entries tagged as adjustments', trend: total > 0 ? 'up' as const : 'neutral' as const },
    { id: 'posted-adjustment', label: 'Posted Adjustment Entries', value: posted, change: 'Committed to the ledger', trend: posted > 0 ? 'up' as const : 'neutral' as const },
    { id: 'draft-adjustment', label: 'Draft Adjustment Entries', value: draft, change: 'Still under validation before posting', trend: draft > 0 ? 'up' as const : 'neutral' as const },
    { id: 'balanced-adjustment', label: 'Balanced Adjustment Entries', value: balanced, change: 'Entries that pass debit-credit balance', trend: balanced > 0 ? 'up' as const : 'neutral' as const },
  ]
}
...
export class AccountingAdjustmentJournalsService {
  static async getRegister(payload: Payload, query: AdjustmentJournalsQuery = {}): Promise<AdjustmentJournalsResult> {
    ...
    const allDocs = await findAllDocs<AdjustmentDoc>({ payload, collection: ACCOUNTING_COLLECTION_SLUGS.journalEntries, depth: 1, sort: '-entryDate' })
    const adjustmentDocs = allDocs.filter((d) => d.sourceType === 'adjustment')
    const sorted = sortEntries(adjustmentDocs)
    ...
    return {
      ...
      totals: { totalEntries: allRows.length, filteredEntries: totalDocs, postedEntries: allRows.filter((r) => r.status === 'posted').length, draftEntries: allRows.filter((r) => r.status === 'draft').length, referencedEntries: allRows.filter((r) => r.referenceNumber).length, balancedEntries: allRows.filter((r) => r.isBalanced === true).length },
    }
  }
}
