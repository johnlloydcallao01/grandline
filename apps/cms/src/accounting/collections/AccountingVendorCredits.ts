...
import { AccountingVendorCreditService } from '../services/bills/AccountingVendorCreditService'
import { applyCreatedAndUpdatedBy } from '../utils/accounting-audit'
import { isTerminalDocumentStatus } from '../utils/commercial'

export const AccountingVendorCredits: CollectionConfig = {
  slug: ACCOUNTING_COLLECTION_SLUGS.vendorCredits,
  admin: {
    useAsTitle: 'vendorCreditNumber',
    defaultColumns: ['vendorCreditNumber', 'vendor', 'creditDate', 'total', 'status'],
    group: ACCOUNTING_ADMIN_GROUP,
    description: 'Vendor credits that reduce payables through posted accounting entries.',
  },
  access: { read: adminOnly, create: adminOnly, update: adminOnly, delete: adminOnly },
  fields: [
    { name: 'vendorCreditNumber', type: 'text', required: true, unique: true, index: true },
    ...
    { name: 'sourceBill', type: 'relationship', relationTo: ACCOUNTING_COLLECTION_SLUGS.bills },
    {
      name: 'applications',
      type: 'array',
      fields: [
  ...
  hooks: {
    beforeChange: [
      async ({ data, req, originalDoc, operation }) => {
        ...
        if (originalDoc && isTerminalDocumentStatus(String(originalDoc.status || ''))) {
          throw new APIError('Posted, settled, or voided vendor credits cannot be edited directly.', 400)
        }
        if (operation === 'create' && !data.vendorCreditNumber) {
          data.vendorCreditNumber = await AccountingVendorCreditService.generateVendorCreditNumber(req.payload)
        }
        AccountingVendorCreditService.normalizeHeader(data)
    ...
    beforeDelete: [
      async ({ req, id }) => {
        const vendorCredit = await req.payload.findByID({
          collection: ACCOUNTING_COLLECTION_SLUGS.vendorCredits,
          id,
          depth: 0,
          overrideAccess: true,
        })
        if (isTerminalDocumentStatus(String(vendorCredit?.status || ''))) {
          throw new APIError('Posted, settled, or voided vendor credits cannot be deleted.', 400)
        }
      },
    ],
  },
