...
export const CourseTags: CollectionConfig = {
  ...
  admin: {
    ...
    description: 'Tags for course content organization and filtering',
  },
  access: {
    read: ({ req: { user } }) => {
      if (user) {
  ...
  fields: [
    {
      ...
      required: true,
      admin: {
        description: 'Tag name (e.g., "Beginner", "Advanced", "STCW")',
      },
    },
    {
      name: 'slug',
      ...
      unique: true,
      admin: {
        description: 'URL-friendly version (e.g., "beginner")',
      },
    },
    {
      name: 'description',
