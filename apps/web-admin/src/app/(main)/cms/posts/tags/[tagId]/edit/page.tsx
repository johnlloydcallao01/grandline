...
import { getTagById, updateTag } from '../../actions';
import PostTagForm from '@/components/cms/PostTagForm';

export default function EditPostTagPage() {
    ...
    const loadTag = useCallback(async () => {
        try {
            ...
            console.error(err);
            setError(err.message?.includes('404') || err.message?.includes('Not Found') ? 'not-found' : (err.message || 'Failed to load tag'));
        } finally {
            setIsLoading(false);
        }
    }, [tagId]);
        ...
        return (
            <div className="p-6 flex items-center justify-center min-h-[400px]">
                <div className="text-center max-w-md">
                    ...
                    <Link href="/cms/posts/tags" className="inline-flex items-center px-4 py-2 bg-blue-600 dark:bg-blue-500 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 text-sm font-medium">
            ...
            <div className="p-6 flex items-center justify-center min-h-[400px]">
                <div className="text-center max-w-md">
                    ...
                    <div className="flex gap-3 justify-center">
                        ...
                        <Link href="/cms/posts/tags" className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 bg-white dark:bg-[var(--card-background)] text-sm font-medium">Back</Link>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="p-6 w-full">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/cms/posts/tags" className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Tag</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{tag?.name}</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <Link href="/cms/posts/tags" className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 bg-white dark:bg-[var(--card-background)]">Cancel</Link>
                    <button form="tag-form" type="submit" disabled={isSaving}
