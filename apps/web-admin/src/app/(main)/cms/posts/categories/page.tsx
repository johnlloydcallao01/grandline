...
import {
    ...
    type PostCategoryDoc
} from './actions';

const ITEMS_PER_PAGE = 20;

export default function PostCategoriesPage() {
    const [categories, setCategories] = useState<PostCategoryDoc[]>([]);
    ...
    const [deleteTarget, setDeleteTarget] = useState<PostCategoryDoc | null>(null);
    const [isDeleting, setIsDeleting] = useState(false);

    const [detailCategory, setDetailCategory] = useState<PostCategoryDoc | null>(null);
    ...
    const openDetail = async (c: PostCategoryDoc) => {
        setDetailCategory(c);
        if (!c.description) {
            setIsDetailLoading(true);
            try {
                const full = await getCategoryById(c.id);
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Post Categories</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Organize blog posts into categories</p>
                </div>
                <Link href="/cms/posts/categories/create"
                ...
                {isLoading ? (
                    ...
                    <>
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/30"><Folder className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
                                <div>
                        ...
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-green-50 dark:bg-green-900/30"><Folder className="h-5 w-5 text-green-600 dark:text-green-400" /></div>
                                <div>
                        ...
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800"><Folder className="h-5 w-5 text-gray-600 dark:text-gray-400" /></div>
                                <div>
                                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{totalDocs - activeCategories}</p>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {categories.map(c => (
                                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className="text-xs font-mono text-gray-600 dark:text-gray-400">{c.slug}</span>
                                        </td>
                                        <td className="px-4 py-3">
                                            {c.colorCode ? (
                                                <div className="flex items-center gap-2">
                                                    <span className="h-4 w-4 rounded border border-gray-200 dark:border-gray-600 inline-block" style={{ backgroundColor: c.colorCode }} />
                                                    <span className="text-xs font-mono text-gray-500 dark:text-gray-400">{c.colorCode}</span>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${c.isActive ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Link href={`/cms/posts/categories/${c.id}/edit`}
                            ...
                            {isDetailLoading ? (
                                ...
                                <>
                                    ...
                                    <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-[var(--card-border)]">
                                        <Link href={`/cms/posts/categories/${detailCategory.id}/edit`}
