...
import {
  BookOpen, Award, Users, GraduationCap,
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm"><TrendsChartSkeleton /></div>
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm" key="skel2"><TrendsChartSkeleton /></div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 [&>:last-child:nth-child(odd)]:col-span-full lg:[&>:last-child:nth-child(odd)]:col-span-1">
        <StatCard title="Total Courses" value={overview.totalCourses} subtitle="On platform" icon={<BookOpen className="w-5 h-5" />} color="blue" formatter={formatNumber} />
        <StatCard title="Enrollments" value={overview.totalEnrollments} subtitle={`${overview.activeEnrollments} active`} icon={<Award className="w-5 h-5" />} color="green" formatter={formatNumber} />
        <StatCard title="Students" value={overview.totalStudents} subtitle="Registered trainees" icon={<Users className="w-5 h-5" />} color="purple" formatter={formatNumber} />
        <StatCard title="Instructors" value={overview.totalInstructors} subtitle="Active instructors" icon={<GraduationCap className="w-5 h-5" />} color="orange" formatter={formatNumber} />
        <StatCard title="Completion Rate" value={overview.completionRate} subtitle={`${overview.completedEnrollments} completed`} icon={<TrendingUp className="w-5 h-5" />} color="teal" formatter={(n) => `${n}%`} />
        <StatCard title="Avg Grade" value={overview.avgGrade} subtitle={`${overview.avgProgress}% avg progress`} icon={<Activity className="w-5 h-5" />} color="pink" formatter={(n) => `${n}%`} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-400">Status breakdown</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-900">Platform Metrics</h3>
            <span className="text-xs text-gray-400">Key indicators</span>
          </div>
          <div className="grid grid-cols-2 gap-4 mt-4">
            {[
              { label: 'Certificates Issued', value: overview.totalCertificates, color: 'bg-purple-500', icon: <Award className="w-4 h-4 text-purple-600" /> },
              { label: 'Assessments Taken', value: overview.totalAssessments, color: 'bg-orange-500', icon: <BarChart3 className="w-4 h-4 text-orange-600" /> },
              { label: 'Assignments Submitted', value: overview.totalAssignments, color: 'bg-teal-500', icon: <Activity className="w-4 h-4 text-teal-600" /> },
              { label: 'Avg Progress', value: overview.avgProgress, color: 'bg-pink-500', icon: <TrendingUp className="w-4 h-4 text-pink-600" /> },
            ].map(item => (
              <div key={item.label} className="bg-gray-50 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="p-1.5 rounded-md bg-white shadow-sm ring-1 ring-black/5">{item.icon}</div>
                  <span className="text-xs text-gray-500 font-medium">{item.label}</span>
                </div>
                <span className="text-xl font-bold text-gray-900 tabular-nums">
                  {item.label === 'Avg Progress' ? `${item.value}%` : formatNumber(item.value)}
                </span>
              </div>
            ))}
          </div>
