...
type TaxBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';
...
type TaxCompliancePageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: TaxComplianceTab[];
  headerActions?: TaxAction[];
};

function getActionIcon(icon?: TaxAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: TaxAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: TaxBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: TaxTableCell, index: number) {
...
function MetricCard({ metric }: { metric: TaxMetric }) {
