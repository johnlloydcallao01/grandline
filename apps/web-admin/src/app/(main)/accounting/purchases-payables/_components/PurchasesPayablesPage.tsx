...
type PayablesBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type PayablesTableCell =
  | string
  | {
      text: string;
      tone?: PayablesBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type PayablesTableRow = {
  id: string;
  cells: PayablesTableCell[];
};

export type PurchasesPayablesTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: PayablesMetric[];
  actions: PayablesAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: PayablesTableRow[];
};

type PurchasesPayablesPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: PurchasesPayablesTab[];
  headerActions?: PayablesAction[];
};

function getActionIcon(icon?: PayablesAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: PayablesAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: PayablesBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: PayablesTableCell, index: number) {
...
function MetricCard({ metric }: { metric: PayablesMetric }) {
