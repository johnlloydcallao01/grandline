    ...
    return (
      <td className="px-4 py-3 text-right">
        <div className="flex justify-end gap-2">
          ...
          <button type="button" onClick={() => setVoidTarget({ id: row.id, receiptNumber: row.receiptNumber })} disabled={!isIssued} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title="Void receipt">
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">Receipt & Payment Review</h1>
          <p className="mt-1 text-base text-gray-600">Review LMS receipts and proof-of-payment coverage for customer payments collected against enrollment billing and invoice settlement.</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-semibold text-gray-900">{receiptData?.meta?.label || currentTab.label}</h2>
              <p className="text-sm text-gray-600">{receiptData?.meta?.description || currentTab.description}</p>
              <p className="text-sm text-gray-500">{receiptData?.totals?.filteredRows ?? 0} matching rows</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" onClick={handleExport} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={!receiptData?.rows?.length}>
            ...
            <div className="space-y-4 p-5">
              {isFilterPanelOpen ? (
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <button type="button" onClick={() => { setDraftFilters({ statuses: [], customerIds: [], proofStates: [] }); setFilters({ statuses: [], customerIds: [], proofStates: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700">Clear all</button>
                    ...
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(receiptData?.filterOptions?.proofStates || []).map((option) => {
                          ...
                          return <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, proofStates: toggleFilterValue(previous.proofStates, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900">{receiptData?.meta?.tableTitle || currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600">{receiptData?.meta?.tableDescription || currentTab.tableDescription}</p>
                </div>
                ...
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            {(receiptData?.meta?.columns || currentTab.columns).map((column) => {
                              ...
                              return (
                                <th key={label} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 ${align}`}>{label}</th>
                              );
                            })}
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {(receiptData?.rows || []).length > 0 ? (receiptData?.rows || []).map((row) => (
                            ...
                            <tr>
                              <td colSpan={(receiptData?.meta?.columns || currentTab.columns).length + 1} className="px-4 py-10 text-center text-sm text-gray-500">No receipt rows found.</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
                  ...
                  {receiptData?.pagination && receiptData.pagination.totalPages > 1 ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600">Page {receiptData.pagination.page} of {receiptData.pagination.totalPages}</p>
                      <div className="flex gap-2">
                        <button type="button" disabled={!receiptData.pagination.hasPrevPage} onClick={() => setCurrentPage((previous) => Math.max(1, previous - 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Previous</button>
                        <button type="button" disabled={!receiptData.pagination.hasNextPage} onClick={() => setCurrentPage((previous) => previous + 1)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Next</button>
                      </div>
                    </div>
                  ) : null}
                </>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" onClick={handleProofExport} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={!receiptData?.rows?.length}>
                <Download className="h-4 w-4" />
                Download View
              </button>
            </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
                <form onSubmit={handleProofSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
                  <div className="relative min-w-0 flex-1">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                    <input type="text" placeholder={currentTab.searchPlaceholder} value={proofSearchInput} onChange={(event) => setProofSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                  </div>
                  <button type="submit" className="inline-flex items-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:border-blue-700 hover:bg-blue-700">
                    <Search className="h-4 w-4" />
                    Search
                ...
                <button type="button" onClick={() => { if (!isProofFilterPanelOpen) setProofDraftFilters({ ...proofFilters }); setIsProofFilterPanelOpen((previous) => !previous); }} className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${isProofFilterPanelOpen || proofFilterCount > 0 ? 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}>
                  <Filter className="h-4 w-4" />
                  Filters
                  {proofFilterCount > 0 ? <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-blue-600 px-1.5 py-0.5 text-[11px] font-semibold text-white">{proofFilterCount}</span> : null}
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {(receiptData?.filterOptions?.quickFilters || []).map((filter) => (
                  <button key={filter.value} type="button" onClick={() => handleToggleProofQuickFilter(filter.value)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${proofQuickFilters.includes(filter.value) ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {filter.label}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4 p-5">
              {isProofFilterPanelOpen ? (
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-900">Filters</h4>
                      <p className="mt-1 text-sm text-gray-600">Select proof coverage and status values to narrow the register view.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <button type="button" onClick={() => { setProofDraftFilters({ statuses: [], customerIds: [], proofStates: [] }); setProofFilters({ statuses: [], customerIds: [], proofStates: [] }); setProofCurrentPage(1); setIsProofFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700">Clear all</button>
                      <button type="button" onClick={() => setIsProofFilterPanelOpen(false)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
                      <button type="button" onClick={() => { setProofFilters({ ...proofDraftFilters }); setProofCurrentPage(1); setIsProofFilterPanelOpen(false); }} className="rounded-lg border border-blue-600 bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">Apply Filters</button>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(receiptData?.filterOptions?.statuses || []).map((option) => {
                          ...
                          return <button key={option.value} type="button" onClick={() => setProofDraftFilters((previous) => ({ ...previous, statuses: toggleFilterValue(previous.statuses, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(receiptData?.filterOptions?.customers || []).map((option) => {
                          ...
                          return <button key={option.value} type="button" onClick={() => setProofDraftFilters((previous) => ({ ...previous, customerIds: toggleFilterValue(previous.customerIds, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900">{currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600">{currentTab.tableDescription}</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                  <span>{receiptData?.totals?.filteredRows ?? 0} matching rows</span>
                </div>
                ...
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {(receiptData?.rows || []).length > 0 ? (receiptData?.rows || []).map((row) => {
                            ...
                            return (
                              <tr key={row.id} className="hover:bg-gray-50">
                                <td className="whitespace-nowrap px-4 py-3 text-sm font-semibold text-gray-900">{row.receiptNumber}</td>
                                <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{row.paymentLabel}</td>
                                <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{row.customerLabel}</td>
                                <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900">{row.proofDocumentLabel || '-'}</td>
                                <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{row.receiptDateLabel}</td>
                                <td className="whitespace-nowrap px-4 py-3 text-sm">
                                  <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${stateTone === 'green' ? 'bg-green-50 text-green-700 ring-green-200' : 'bg-amber-50 text-amber-700 ring-amber-200'}`}>{stateLabel}</span>
                                </td>
                            ...
                            <tr>
                              <td colSpan={currentTab.columns.length + 1} className="px-4 py-10 text-center text-sm text-gray-500">No proof of payment rows found.</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
                  ...
                  {receiptData?.pagination && receiptData.pagination.totalPages > 1 ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600">Page {receiptData.pagination.page} of {receiptData.pagination.totalPages}</p>
                      <div className="flex gap-2">
                        <button type="button" disabled={!receiptData.pagination.hasPrevPage} onClick={() => setProofCurrentPage((previous) => Math.max(1, previous - 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Previous</button>
                        <button type="button" disabled={!receiptData.pagination.hasNextPage} onClick={() => setProofCurrentPage((previous) => previous + 1)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Next</button>
                      </div>
                    </div>
                  ) : null}
                </>
      ...
      <SlideOver isOpen={Boolean(issueTarget)} onClose={() => setIssueTarget(null)} title="Issue Receipt" description="Issuing finalizes the receipt and links it to the payment received record." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            <button type="button" onClick={() => setIssueTarget(null)} disabled={isIssuing} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 disabled:opacity-50">Cancel</button>
            <button type="button" onClick={handleConfirmIssue} disabled={isIssuing} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">{isIssuing ? 'Issuing...' : 'Issue Receipt'}</button>
          </div>
        </div>
      </SlideOver>
      ...
      <SlideOver isOpen={Boolean(voidTarget)} onClose={() => setVoidTarget(null)} title="Void Receipt" description="Voiding marks the receipt as void but retains it for audit visibility." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            ...
            <button type="button" onClick={handleConfirmVoid} disabled={isVoiding} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50">{isVoiding ? 'Voiding...' : 'Void Receipt'}</button>
          </div>
        </div>
      </SlideOver>
      ...
      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Receipt" description="Delete this mutable receipt after dependency validation completes." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Receipt'}</button>
          </div>
        </div>
      </SlideOver>
    </div>
