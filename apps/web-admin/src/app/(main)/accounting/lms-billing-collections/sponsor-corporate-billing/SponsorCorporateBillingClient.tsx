  ...
  const renderActions = (row: { id: string; status?: string | null }) => (
    <td className="px-4 py-3 text-right">
      <div className="flex justify-end gap-2">
        ...
        <button type="button" onClick={() => handleOpenEdit(row.id)} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700" title="Edit record">
          <Edit className="h-4 w-4" />
        </button>
        <button type="button" onClick={() => setDeleteTarget({ id: row.id, label: row.id })} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700" title="Delete record">
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">Sponsor & Corporate Billing</h1>
          <p className="mt-1 text-base text-gray-600">Review scholarship awards and corporate billing links that reduce or transfer LMS trainee charges to sponsor and company payers.</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-semibold text-gray-900">{awardData?.section?.label || currentTab.label}</h2>
              <p className="text-sm text-gray-600">{awardData?.section?.description || currentTab.description}</p>
              <p className="text-sm text-gray-500">{awardData?.totals?.filteredAwards ?? 0} matching rows</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" onClick={handleAwardExport} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={!currentRows.length}>
              ...
              <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
                <form onSubmit={handleAwardSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
                  <div className="relative min-w-0 flex-1">
                    ...
                    <input type="text" placeholder={awardData?.section?.searchPlaceholder || currentTab.searchPlaceholder} value={awardSearchInput} onChange={(event) => setAwardSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                  </div>
                ...
                <button type="button" onClick={() => { if (!isAwardFilterPanelOpen) setAwardDraftFilters({ ...awardFilters }); setIsAwardFilterPanelOpen((previous) => !previous); }} className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${isAwardFilterPanelOpen || awardFilterCount > 0 ? 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}>
                  <Filter className="h-4 w-4" />
                  Filters
                  {awardFilterCount > 0 ? <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-blue-600 px-1.5 py-0.5 text-[11px] font-semibold text-white">{awardFilterCount}</span> : null}
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {(awardData?.section?.filters || []).map((filter) => (
                  <button key={filter.value} type="button" onClick={() => { const next = toggleFilterValue(awardFilters.statuses, filter.value); setAwardFilters({ statuses: next }); setAwardCurrentPage(1); }} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${awardFilters.statuses.includes(filter.value) ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {filter.label}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4 p-[10px] sm:p-5">
                ...
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600">Select status values to narrow the award register.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <button type="button" onClick={() => { setAwardDraftFilters({ statuses: [] }); setAwardFilters({ statuses: [] }); setAwardCurrentPage(1); setIsAwardFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700">Clear all</button>
                      <button type="button" onClick={() => setIsAwardFilterPanelOpen(false)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
                      <button type="button" onClick={() => { setAwardFilters({ ...awardDraftFilters }); setAwardCurrentPage(1); setIsAwardFilterPanelOpen(false); }} className="rounded-lg border border-blue-600 bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">Apply Filters</button>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(awardData?.section?.filters || []).map((option) => {
                          ...
                          return <button key={option.value} type="button" onClick={() => setAwardDraftFilters((previous) => ({ ...previous, statuses: toggleFilterValue(previous.statuses, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900">{awardData?.section?.table?.title || currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600">{awardData?.section?.table?.description || currentTab.tableDescription}</p>
                </div>
                ...
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {currentRows.length > 0 ? currentRows.map((row) => (
                            ...
                            <tr>
                              <td colSpan={(awardData?.section?.table?.columns || currentTab.columns).length + 1} className="px-4 py-10 text-center text-sm text-gray-500">No scholarship award rows found.</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
                  ...
                  {currentPagination && currentPagination.totalPages > 1 ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600">Page {currentPagination.page} of {currentPagination.totalPages}</p>
                      <div className="flex gap-2">
                        <button type="button" disabled={!currentPagination.hasPrevPage} onClick={() => setAwardCurrentPage((previous) => Math.max(1, previous - 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Previous</button>
                        <button type="button" disabled={!currentPagination.hasNextPage} onClick={() => setAwardCurrentPage((previous) => previous + 1)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Next</button>
                      </div>
                    </div>
                  ) : null}
                </>
      ...
      {activeTab === 'corporate-billing-links' && (
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-semibold text-gray-900">{linkData?.section?.label || currentTab.label}</h2>
              <p className="text-sm text-gray-600">{linkData?.section?.description || currentTab.description}</p>
              <p className="text-sm text-gray-500">{linkData?.totals?.filteredLinks ?? 0} matching rows</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={handleOpenCreate} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('primary')}`}>
                <Plus className="h-4 w-4" />
              ...
              <button type="button" onClick={handleLinkExport} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={!currentRows.length}>
                <Download className="h-4 w-4" />
                Download View
              </button>
            </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
                <form onSubmit={handleLinkSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
                  <div className="relative min-w-0 flex-1">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                    <input type="text" placeholder={linkData?.section?.searchPlaceholder || currentTab.searchPlaceholder} value={linkSearchInput} onChange={(event) => setLinkSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                  </div>
                  <button type="submit" className="inline-flex items-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:border-blue-700 hover:bg-blue-700">
                    <Search className="h-4 w-4" />
                    Search
                ...
                <button type="button" onClick={() => { if (!isLinkFilterPanelOpen) setLinkDraftFilters({ ...linkFilters }); setIsLinkFilterPanelOpen((previous) => !previous); }} className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${isLinkFilterPanelOpen || linkFilterCount > 0 ? 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}>
                  <Filter className="h-4 w-4" />
                  Filters
                  {linkFilterCount > 0 ? <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-blue-600 px-1.5 py-0.5 text-[11px] font-semibold text-white">{linkFilterCount}</span> : null}
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-4 p-[10px] sm:p-5">
              {isLinkFilterPanelOpen ? (
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-900">Filters</h4>
                      <p className="mt-1 text-sm text-gray-600">Select status values to narrow the corporate billing links register.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <button type="button" onClick={() => { setLinkDraftFilters({ statuses: [] }); setLinkFilters({ statuses: [] }); setLinkCurrentPage(1); setIsLinkFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700">Clear all</button>
                      <button type="button" onClick={() => setIsLinkFilterPanelOpen(false)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
                      <button type="button" onClick={() => { setLinkFilters({ ...linkDraftFilters }); setLinkCurrentPage(1); setIsLinkFilterPanelOpen(false); }} className="rounded-lg border border-blue-600 bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">Apply Filters</button>
                    </div>
                  </div>
                  <div className="mt-6 grid gap-6 md:grid-cols-1">
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(linkData?.section?.filters || []).map((option) => {
                          ...
                          return <button key={option.value} type="button" onClick={() => setLinkDraftFilters((previous) => ({ ...previous, statuses: toggleFilterValue(previous.statuses, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900">{linkData?.section?.table?.title || currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600">{linkData?.section?.table?.description || currentTab.tableDescription}</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                ...
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            {(linkData?.section?.table?.columns || currentTab.columns).map((column) => (
                              <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 ${column === 'Covered Amount' || column === 'Trainee Share' ? 'text-right' : 'text-left'}`}>{column}</th>
                            ))}
                            <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Actions</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {currentRows.length > 0 ? currentRows.map((row) => (
                            ...
                            <tr>
                              <td colSpan={(linkData?.section?.table?.columns || currentTab.columns).length + 1} className="px-4 py-10 text-center text-sm text-gray-500">No corporate billing link rows found.</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
                  ...
                  {currentPagination && currentPagination.totalPages > 1 ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600">Page {currentPagination.page} of {currentPagination.totalPages}</p>
                      <div className="flex gap-2">
                        <button type="button" disabled={!currentPagination.hasPrevPage} onClick={() => setLinkCurrentPage((previous) => Math.max(1, previous - 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Previous</button>
                        <button type="button" disabled={!currentPagination.hasNextPage} onClick={() => setLinkCurrentPage((previous) => previous + 1)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50">Next</button>
                      </div>
                    </div>
                  ) : null}
                </>
      ...
      >
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            <button type="button" onClick={() => setIsFormOpen(false)} className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">Cancel</button>
            <button type="submit" disabled={isSubmitting} className="rounded-lg border border-blue-600 bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteTarget(null)}>
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setDeleteTarget(null)} className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">Cancel</button>
              <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg border border-red-600 bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:opacity-50">
