...
type LmsBillingAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type LmsBillingMetric = {
  label: string;
  value: string;
  change: string;
...
type LmsBillingBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type LmsBillingTableCell =
  | string
  | {
      text: string;
      tone?: LmsBillingBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type LmsBillingTableRow = {
  id: string;
  cells: LmsBillingTableCell[];
};

export type LmsBillingCollectionsTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: LmsBillingMetric[];
  actions: LmsBillingAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: LmsBillingTableRow[];
};

type LmsBillingCollectionsPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: LmsBillingCollectionsTab[];
  headerActions?: LmsBillingAction[];
};

function getActionIcon(icon?: LmsBillingAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: LmsBillingAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: LmsBillingBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: LmsBillingTableCell, index: number) {
...
function MetricCard({ metric }: { metric: LmsBillingMetric }) {
...
export function LmsBillingCollectionsPage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: LmsBillingCollectionsPageProps) {
