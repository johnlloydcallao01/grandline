...
type BudgetsForecastsAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type BudgetsForecastsMetric = {
  label: string;
  value: string;
  change: string;
...
type BudgetsForecastsBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type BudgetsForecastsTableCell =
  | string
  | {
      text: string;
      tone?: BudgetsForecastsBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type BudgetsForecastsTableRow = {
  id: string;
  cells: BudgetsForecastsTableCell[];
};

export type BudgetsForecastsTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: BudgetsForecastsMetric[];
  actions: BudgetsForecastsAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: BudgetsForecastsTableRow[];
};

type BudgetsForecastsPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: BudgetsForecastsTab[];
  headerActions?: BudgetsForecastsAction[];
};

function getActionIcon(icon?: BudgetsForecastsAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: BudgetsForecastsAction['variant'] = 'secondary') {
  if (variant === 'primary') {
    return 'border border-blue-600 bg-blue-600 text-white hover:border-blue-700 hover:bg-blue-700';
  }
  if (variant === 'ghost') {
...
function getBadgeClasses(tone: BudgetsForecastsBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: BudgetsForecastsTableCell, index: number) {
  ...
  if (cell.tone) {
    return (
      <td key={index} className={`whitespace-nowrap px-4 py-3 text-sm ${alignClass}`}>
        <span
          className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${getBadgeClasses(cell.tone)}`}
        >
          {cell.text}
        </span>
      </td>
...
function MetricCard({ metric }: { metric: BudgetsForecastsMetric }) {
...
export function BudgetsForecastsPage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: BudgetsForecastsPageProps) {
