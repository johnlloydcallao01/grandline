...
type AuditBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';
...
type AuditTableRow = {
  id: string;
  cells: AuditTableCell[];
};

export type AuditHistoryTab = {
  id: string;
  label: string;
  description: string;
...
type AuditHistoryPageProps = {
  ...
  headerActions?: AuditAction[];
};

function getActionIcon(icon?: AuditAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: AuditAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: AuditBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: AuditTableCell, index: number) {
