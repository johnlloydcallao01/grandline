...
import {
  ...
  type DepreciationCell,
  type DepreciationDetail,
  type DepreciationMetric,
  type DepreciationMutationInput,
  type DepreciationsResponse,
} from './actions-depreciation';
...
function getMetricTone(trend: DepreciationMetric['trend']) {
...
function renderCell(cell: DepreciationCell, index: number) {
...
function buildFormFromDetail(d: DepreciationDetail): FormState {
...
function toMutationInput(f: FormState): DepreciationMutationInput {
  return {
...
function renderForm(
  ...
  refData?: DepreciationsResponse['referenceData'],
) {
  ...
  const periodOpts = [{ label: 'Select a period', value: '' }].concat(
...
export default function DepreciationSchedulesClient() {
  const [data, setData] = useState<DepreciationsResponse | null>(null);
  ...
  const [viewDetail, setViewDetail] = useState<DepreciationDetail | null>(null);
  ...
  const fetchData = useCallback(async (search: string, page: number, nextFilters: FilterState, nextQuickFilters: string[]) => {
    ...
    setErrorState(null);
    try {
      const response = await getDepreciationSchedules({
        search,
        page,
      ...
      setErrorState(fetchError instanceof Error ? fetchError.message : 'Unable to load depreciation schedules.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleExport = () => {
    ...
    const csvRows = rows.map((row) => [row.assetLabel, row.fiscalYearLabel, row.periodLabel, row.depreciationDate || '', row.amountLabel, row.statusLabel]);
  ...
  const handleView = async (id: string) => {
    ...
    setViewDetail(null);
    try {
      setViewDetail(await getDepreciationSchedule(id));
    } catch {
      // handled silently
    } finally {
  ...
  const handleCreateSubmit = async (event: React.FormEvent) => {
    ...
    setCreateError(null);
    try {
      await createDepreciationSchedule(toMutationInput(createForm));
      setIsCreateOpen(false);
      handleRefresh();
    } catch (submissionError) {
      setCreateError(submissionError instanceof Error ? submissionError.message : 'Unable to create depreciation schedule.');
    } finally {
  ...
  const handleOpenEdit = async (id: string) => {
    ...
    try {
      const detail = await getDepreciationSchedule(id);
      setEditForm(buildFormFromDetail(detail));
    } catch (detailError) {
      setEditError(detailError instanceof Error ? detailError.message : 'Unable to load depreciation schedule.');
    } finally {
  ...
  const handleEditSubmit = async (event: React.FormEvent) => {
    ...
    setEditError(null);
    try {
      await updateDepreciationSchedule(editId, toMutationInput(editForm));
      setIsEditOpen(false);
      setEditId(null);
      handleRefresh();
    } catch (submissionError) {
      setEditError(submissionError instanceof Error ? submissionError.message : 'Unable to update depreciation schedule.');
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleteError(null);
    try {
      await deleteDepreciationSchedule(deleteId);
      setIsDeleteOpen(false);
      setDeleteId(null);
      setDeleteLabel('');
      handleRefresh();
    } catch (deletionError) {
      setDeleteError(deletionError instanceof Error ? deletionError.message : 'Unable to delete depreciation schedule.');
    } finally {
  ...
  return (
    <div className="space-y-6 p-[10px]">
          ...
          {isFilterPanelOpen ? (
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters({ statuses: [] }); setFilters({ statuses: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700">
                    Clear all
                  </button>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Depreciation Schedule Detail" description="Full depreciation schedule entry including asset, fiscal year, period, amount, and status.">
      ...
      <SlideOver isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="New Depreciation Schedule Entry" description="Create a depreciation schedule entry. All required fields must be completed.">
      ...
      <SlideOver isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Depreciation Schedule Entry" description="Update the depreciation schedule entry fields.">
      ...
      <SlideOver isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} title="Delete Depreciation Schedule Entry" description="Remove this depreciation schedule entry.">
        <div className="space-y-6">
          {deleteError ? (
            <div className="flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700">
              <AlertCircle className="h-4 w-4 shrink-0" />
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            <p className="font-medium">Delete schedule entry for {deleteLabel}?</p>
