...
type FixedAssetsAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type FixedAssetsMetric = {
  label: string;
  value: string;
  change: string;
...
type FixedAssetsBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type FixedAssetsTableCell =
  | string
  | {
      text: string;
      tone?: FixedAssetsBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type FixedAssetsTableRow = {
  id: string;
  cells: FixedAssetsTableCell[];
};

export type FixedAssetsTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: FixedAssetsMetric[];
  actions: FixedAssetsAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: FixedAssetsTableRow[];
};
...
type FixedAssetsPageProps = {
  ...
  description: string;
  tabs: FixedAssetsTab[];
  headerActions?: FixedAssetsAction[];
};

function getActionIcon(icon?: FixedAssetsAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: FixedAssetsAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: FixedAssetsBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: FixedAssetsTableCell, index: number) {
...
function MetricCard({ metric }: { metric: FixedAssetsMetric }) {
