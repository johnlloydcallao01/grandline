  ...
  const renderForm = (
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
        <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Posting Guidance</h4>
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Draft transfers can be reviewed and edited here, then posted later from the table using the dedicated Post action.</p>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={handleRefresh} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('secondary')} dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700`}>
        ...
        <div className="space-y-4 p-5">
            ...
            <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Select as many filter values as needed. Transfer filters widen results using OR behavior across all checked options.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters({ statuses: [], bankAccounts: [], coverageStates: [] }); setFilters({ statuses: [], bankAccounts: [], coverageStates: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                    Clear all
                  </button>
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-3">
                ...
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Accounts</h5>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.bankAccounts || []).map((option) => {
                      const isSelected = draftFilters.bankAccounts.includes(option.value);
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, bankAccounts: toggleFilterValue(previous.bankAccounts, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 dark:bg-blue-700 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.coverageStates || []).map((option) => {
                      ...
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, coverageStates: toggleFilterValue(previous.coverageStates, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 dark:bg-blue-700 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta.tableTitle || 'Transfer Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.tableDescription || 'Live register of cash transfers and undeposited-funds movements.'}</p>
            </div>
                          ...
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                <button type="button" onClick={() => handleOpenView(row.id)} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300" title="View detail">
                                  <Eye className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenPost(row)} disabled={!row.isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 dark:hover:text-blue-400 disabled:cursor-not-allowed disabled:opacity-40" title={row.isDraft ? 'Post transfer' : 'Only draft transfers can be posted'}>
                                  <SendHorizonal className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!row.isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:cursor-not-allowed disabled:opacity-40" title={row.isDraft ? 'Edit' : 'Only draft transfers can be edited'}>
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenDelete(row)} disabled={!row.isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title={row.isDraft ? 'Delete' : 'Only draft transfers can be deleted'}>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Transfer Detail" description="Review origin, destination, journal linkage, and undeposited-funds involvement for the selected transfer.">
        {isViewLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className={`rounded-xl border p-4 shadow-sm ${viewDetail.involvesUndepositedFunds ? 'border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30' : 'border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)]'}`}>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Undeposited Funds</p>
                <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.involvesUndepositedFunds ? 'This transfer involves an undeposited-funds account.' : 'This transfer does not involve an undeposited-funds account.'}</p>
              </div>
              <div className={`rounded-xl border p-4 shadow-sm ${viewDetail.isBankToBank ? 'border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/30' : 'border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)]'}`}>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Transfer Shape</p>
                <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.isBankToBank ? 'Bank-to-bank transfer' : 'Mixed account-type transfer'}</p>
              </div>
            </div>
      ...
      <SlideOver isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} title="Delete Transfer" description="Delete this draft transfer only if it has not been posted.">
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setIsDeleteOpen(false)} disabled={isDeleteSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 dark:bg-[var(--card-background)] dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleteSubmitting || (deleteDetail ? !deleteDetail.usageSummary.canDelete : true)} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:opacity-50">
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Transfer No.</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{postTarget.transferNumber}</span>
                </div>
                <div className="mt-3 flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">From Account</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{postTarget.fromBankAccountLabel}</span>
                </div>
                <div className="mt-3 flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">To Account</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{postTarget.toBankAccountLabel}</span>
                </div>
                <div className="mt-3 flex justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Amount</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{postTarget.amountLabel}</span>
                </div>
              </div>
            </>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">Loading transfer detail...</p>
          )}

          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setIsPostOpen(false)} disabled={isPostSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 dark:bg-[var(--card-background)] dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmPost} disabled={isPostSubmitting || !postTarget?.usageSummary.canPost} className="rounded-lg bg-blue-600 dark:bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800 disabled:opacity-50">
