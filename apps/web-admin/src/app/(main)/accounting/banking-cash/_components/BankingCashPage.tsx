...
type BankingBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type BankingTableCell =
  | string
  | {
      text: string;
      tone?: BankingBadgeTone;
      emphasis?: boolean;
...
type BankingTableRow = {
  id: string;
  cells: BankingTableCell[];
};

type BankingPanelItem = {
  label: string;
  value: string;
  tone?: BankingBadgeTone;
};
...
type BankingPanel = {
  title: string;
  subtitle?: string;
  items: BankingPanelItem[];
};

export type BankingCashTab = {
  id: string;
  label: string;
  description: string;
  ...
  columns: string[];
  rows: BankingTableRow[];
  sidePanels: BankingPanel[];
  footerPanels?: BankingPanel[];
};

type BankingCashPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: BankingCashTab[];
  headerActions?: BankingAction[];
};

function getActionIcon(icon?: BankingAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: BankingAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: BankingBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: BankingTableCell, index: number) {
...
function MetricCard({ metric }: { metric: BankingMetric }) {
...
function PanelCard({ panel }: { panel: BankingPanel }) {
  return (
...
export function BankingCashPage({ eyebrow, title, description, tabs, headerActions }: BankingCashPageProps) {
