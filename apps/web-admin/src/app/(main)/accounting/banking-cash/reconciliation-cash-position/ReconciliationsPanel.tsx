    ...
    >
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-gray-200 dark:border-[var(--card-border)] px-6 py-4">
          ...
          <button type="button" onClick={onClose} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-200">
            <X className="h-5 w-5" />
          </button>
        </div>
  ...
  const renderForm = (
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
        <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Snapshot Guidance</h4>
        ...
        <div className="space-y-4 p-5">
          {isFilterPanelOpen ? (
            <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card-background)] p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters({ statuses: [], bankAccountIds: [], differenceStates: [] }); setFilters({ statuses: [], bankAccountIds: [], differenceStates: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                    Clear all
                  </button>
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-3">
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.statuses || []).map((option) => {
                      ...
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, statuses: toggleFilterValue(previous.statuses, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.bankAccounts || []).map((option) => {
                      ...
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, bankAccountIds: toggleFilterValue(previous.bankAccountIds, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
                ...
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Difference</h5>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.differenceStates || []).map((option) => {
                      const isSelected = draftFilters.differenceStates.includes(option.value);
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, differenceStates: toggleFilterValue(previous.differenceStates, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta.tableTitle || 'Reconciliation Sessions'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.tableDescription || 'Live register of reconciliation sessions and their current variance.'}</p>
            </div>
              ...
              <div className="overflow-hidden rounded-xl border border-[var(--card-border)]">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-[var(--card-border)]">
                    ...
                    <tbody className="divide-y divide-[var(--card-border)] bg-white dark:bg-[var(--card-background)]">
                      {(data?.rows || []).length > 0 ? (
                        (data?.rows || []).map((row) => (
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                <button type="button" onClick={() => handleOpenView(row.id)} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-200" title="View detail">
                                  <Eye className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenComplete(row)} disabled={row.isCompleted || row.isLocked} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 dark:hover:text-blue-300 disabled:cursor-not-allowed disabled:opacity-40" title={row.isCompleted || row.isLocked ? 'Completed or locked sessions cannot be completed again' : 'Complete reconciliation'}>
                                  <SendHorizonal className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={row.isCompleted || row.isLocked} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-200 disabled:cursor-not-allowed disabled:opacity-40" title={row.isCompleted || row.isLocked ? 'Completed or locked sessions cannot be edited' : 'Edit'}>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Reconciliation Detail" description="Review the current snapshot, statement balances, variance, and bank-transaction matching progress for this reconciliation session.">
        {isViewLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
              <div className="flex flex-col gap-2 border-b border-gray-200 dark:border-[var(--card-border)] pb-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Matching Snapshot</h4>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                ...
                <div className="rounded-full bg-gray-100 dark:bg-gray-800 px-3 py-1 text-xs font-medium text-gray-700 dark:text-gray-300">
                  Net Activity: {viewDetail.snapshot.statementActivityNetLabel}
                </div>
              </div>

              {viewDetail.snapshot.rows.length > 0 ? (
                <div className="mt-4 overflow-hidden rounded-xl border border-[var(--card-border)]">
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-[var(--card-border)]">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                ...
                <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">No bank transactions fall inside the selected statement period yet.</p>
              )}
            </div>
          </div>
        ) : (
        ...
        <div className="space-y-6">
          ...
          {isCompleteLoading ? (
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <div className="flex justify-between border-b border-gray-200 dark:border-[var(--card-border)] pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Session</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{completeTarget.sessionLabel}</span>
                </div>
                <div className="mt-3 flex justify-between border-b border-gray-200 dark:border-[var(--card-border)] pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Bank Account</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{completeTarget.bankAccountLabel}</span>
                </div>
                <div className="mt-3 flex justify-between border-b border-gray-200 dark:border-[var(--card-border)] pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Statement Period</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{completeTarget.statementPeriodLabel}</span>
                </div>
                <div className="mt-3 flex justify-between border-b border-gray-200 dark:border-[var(--card-border)] pb-3">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Difference</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{completeTarget.snapshot.differenceLabel}</span>
                </div>
                <div className="mt-3 flex justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Unmatched Transactions</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{completeTarget.snapshot.unmatchedTransactionCount}</span>
                </div>
              </div>
            </>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">Loading reconciliation detail...</p>
          )}

          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setIsCompleteOpen(false)} disabled={isCompleteSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmComplete} disabled={isCompleteSubmitting || !completeTarget?.usageSummary.canComplete} className="rounded-lg bg-blue-600 dark:bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800 disabled:opacity-50">
