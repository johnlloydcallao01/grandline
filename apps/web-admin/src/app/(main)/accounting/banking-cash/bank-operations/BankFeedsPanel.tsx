...
function SlideOver({
  ...
  return createPortal(
    ...
    >
      <div
        className={`flex w-full max-w-2xl flex-col bg-white shadow-xl transition-all duration-300 ease-in-out dark:bg-[var(--card-background)] ${animate ? 'translate-x-0' : 'translate-x-full'}`}
...
function Select({
  ...
  return (
    <select
      ...
      onChange={(event) => onChange(event.target.value)}
      required={required}
      className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-[var(--card-background)] dark:text-gray-100 dark:focus:border-blue-600 dark:focus:ring-blue-900/50 dark:disabled:bg-gray-800/50"
    >
      {options.map((option) => (
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <FormField label="Needs Reconnection">
          <label className="inline-flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 dark:border-gray-600 dark:text-gray-300">
      ...
      <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-[var(--card-border)] dark:bg-gray-800/50">
        <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">System Sync Timeline</h4>
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">These operational timestamps stay visible for accountants but are read-only in the slide-over.</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-[var(--card-border)] dark:bg-gray-800/50">
        <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Operational Preview</h4>
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div className="rounded-lg border border-gray-200 bg-white px-3 py-3 dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Connection</p>
            <p className="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">{formState.connectionStatus || '-'}</p>
          </div>
          <div className="rounded-lg border border-gray-200 bg-white px-3 py-3 dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Health</p>
            <p className="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">{formState.healthStatus || '-'}</p>
          </div>
          <div className="rounded-lg border border-gray-200 bg-white px-3 py-3 dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Feed Rules</p>
            <p className="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">{formState.feedRuleCount || '0'}</p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between dark:border-[var(--card-border)] dark:bg-gray-800/50">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{data?.meta.label || 'Bank Feeds'}</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.description || 'Monitor connected accounts, feed health, sync history, and feed rules.'}</p>
        ...
        <div className="space-y-4 p-5">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-[var(--card-border)] dark:bg-gray-800/50">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Select as many filter values as needed. Bank feed filters widen results using OR behavior across all checked options.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters({ statuses: [], connectors: [], healthStates: [] }); setFilters({ statuses: [], connectors: [], healthStates: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
                    Clear all
                  </button>
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-3">
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Connection Status</h5>
                ...
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Connector</h5>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.connectors || []).map((option) => {
                      const isSelected = draftFilters.connectors.includes(option.value);
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, connectors: toggleFilterValue(previous.connectors, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 text-white dark:bg-blue-700' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-600 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
                ...
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Health Status</h5>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.healthStates || []).map((option) => {
                      const isSelected = draftFilters.healthStates.includes(option.value);
                      return (
                        <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, healthStates: toggleFilterValue(previous.healthStates, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 text-white dark:bg-blue-700' : 'bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-600 dark:hover:bg-gray-700'}`}>
                          {option.label}
                        </button>
                      );
                    })}
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta.tableTitle || 'Connected Feed Accounts'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.tableDescription || 'Operational status of each connected bank feed and the rules attached to it.'}</p>
            </div>
        ...
        {isViewLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
              <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Feed Rule Set</h4>
              <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{viewDetail.feedRuleSetName || '-'}</p>
            </div>

            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
              <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Disconnect Reason</h4>
              <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{viewDetail.disconnectReason || '-'}</p>
            </div>

            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
              <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Last Error Summary</h4>
              <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{viewDetail.lastErrorSummary || '-'}</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-[var(--card-border)] dark:bg-[var(--card-background)]">
                ...
                <div className="mt-3 flex justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{deleteDetail.connectionStatusLabel}</span>
                </div>
                <div className="mt-3 flex justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Last Sync</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{deleteDetail.lastSyncAtLabel}</span>
                </div>
              </div>
            </div>
          ) : null}
