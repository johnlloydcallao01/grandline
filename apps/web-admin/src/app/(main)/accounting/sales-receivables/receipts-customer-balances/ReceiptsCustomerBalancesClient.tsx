                    ...
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {tab.columns.map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Credit Limit' || column === 'Balance Due' ? 'text-right' : 'text-left'}`}>
                            {column}
                          </th>
                        ))}
                          ...
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" onClick={() => handleOpenEdit(row.id)} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300" title="Edit customer">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => handleOpenDelete(row.id, row.customerLabel)} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700" title="Delete customer">
            ...
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Contacts</p>
                ...
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Dependencies</p>
                  <div className="mt-3 space-y-2 text-sm text-gray-700 dark:text-gray-300">
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
                <div className="border-b border-gray-200 dark:border-gray-800 px-5 py-4">
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Open Invoices</h4>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Invoice', 'Invoice Date', 'Due Date', 'Balance Due', 'Status'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Balance Due' ? 'text-right' : 'text-left'}`}>
                            {column}
                          </th>
                        ))}
                      </tr>
                      ...
                      {viewDetail.openInvoices.length > 0 ? (
                        viewDetail.openInvoices.map((invoice) => (
                          <tr key={invoice.id}>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{invoice.invoiceDateLabel}</td>
                            <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{invoice.dueDateLabel}</td>
                            <td className="px-4 py-3 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">{invoice.balanceDueLabel}</td>
                            <td className="px-4 py-3 text-sm">
                              <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${invoice.statusTone === 'green' ? 'bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400 ring-green-200 dark:ring-green-800' : invoice.statusTone === 'amber' ? 'bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 ring-amber-200 dark:ring-amber-800' : invoice.statusTone === 'red' ? 'bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 ring-red-200 dark:ring-red-800' : invoice.statusTone === 'blue' ? 'bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 ring-blue-200 dark:ring-blue-800' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400 ring-gray-200 dark:ring-gray-700'}`}>
                                {invoice.statusLabel}
                              </span>
                            </td>
                          </tr>
                        ...
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No open invoices found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Customer Balance Source" description="Update the customer master fields that drive receivable monitoring. Balance figures remain visible but read-only.">
        <form onSubmit={handleSubmitEdit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setIsEditOpen(false)} disabled={isEditSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
        ...
        <div className="space-y-6">
          {isDeleteLoading ? (
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 text-sm text-gray-700 dark:text-gray-300">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setDeleteTarget(null)} disabled={isDeleting} className="rounded-lg bg-gray-100 dark:bg-[var(--card-background)] px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting || isDeleteLoading || !deleteDetail?.usageSummary.canDelete} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:cursor-not-allowed disabled:opacity-50">
    ...
    <div className="space-y-6">
                            ...
                            <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                              ...
                              <td className="px-4 py-3 text-right">
                                <div className="flex justify-end gap-2">
                                  ...
                                  <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:cursor-not-allowed disabled:opacity-40" title="Edit receipt">
                                    <Edit className="h-4 w-4" />
                                  </button>
                                  <button type="button" onClick={() => setIssueTarget({ id: row.id, receiptNumber: row.receiptNumber })} disabled={!isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-40" title="Issue receipt">
                                    <Send className="h-4 w-4" />
                                  </button>
                                  <button type="button" onClick={() => setVoidTarget({ id: row.id, receiptNumber: row.receiptNumber })} disabled={!isIssued} className="inline-flex items-center gap-1 rounded-lg p-2 text-amber-600 dark:text-amber-400 transition-colors hover:bg-amber-50 dark:hover:bg-amber-950/30 hover:text-amber-700 disabled:cursor-not-allowed disabled:opacity-40" title="Void receipt">
                                    <Ban className="h-4 w-4" />
                                  </button>
                                  <button type="button" onClick={() => setDeleteTarget({ id: row.id, receiptNumber: row.receiptNumber })} disabled={!isDraft} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title="Delete receipt">
                                    <Trash2 className="h-4 w-4" />
                                  </button>
                                </div>
                              </td>
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                ...
                {viewDetail.proofDocumentUrl ? (
                  <a href={viewDetail.proofDocumentUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700">
                    Open Proof Document
                  </a>
                ) : null}
              </div>
      ...
      <SlideOver isOpen={Boolean(issueTarget)} onClose={() => setIssueTarget(null)} title="Issue Official Receipt" description="Issuing finalizes the official receipt and locks direct edits on the record." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setIssueTarget(null)} disabled={isIssuing} className="rounded-lg bg-gray-100 dark:bg-[var(--card-background)] px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmIssue} disabled={isIssuing} className="rounded-lg bg-blue-600 dark:bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800 disabled:opacity-50">
      ...
      <SlideOver isOpen={Boolean(voidTarget)} onClose={() => setVoidTarget(null)} title="Void Official Receipt" description="Voiding preserves the receipt for audit purposes while marking it unusable." width="max-w-lg">
        <div className="space-y-6">
          <div className="rounded-xl border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 p-4 text-sm text-amber-700 dark:text-amber-400">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setVoidTarget(null)} disabled={isVoiding} className="rounded-lg bg-gray-100 dark:bg-[var(--card-background)] px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmVoid} disabled={isVoiding} className="rounded-lg bg-amber-600 dark:bg-amber-700 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700 dark:hover:bg-amber-800 disabled:opacity-50">
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">Receipts & Customer Balances</h1>
      ...
      {activeTab === 'official-receipts' ? (
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{data?.meta.label || currentTab.label}</h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.description || currentTab.description}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">{data?.totals.filteredRows ?? 0} matching rows</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={handleOpenCreate} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('primary')}`}>
          ...
          <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
            ...
            <div className="space-y-4 p-[10px] sm:p-5">
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta.tableTitle || currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.tableDescription || currentTab.tableDescription}</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                  <span>{data?.totals.filteredRows ?? 0} matching rows</span>
                </div>
                                ...
                                <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                  ...
                                  <td className="px-4 py-3 text-right">
                                    <div className="flex justify-end gap-2">
                                      ...
                                      <button type="button" onClick={() => setPostTarget({ id: row.id, receiptNumber: row.receiptNumber })} disabled={!canPost} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-40" title="Post receipt">
                                        <Send className="h-4 w-4" />
                                      </button>
                    ...
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Invoice', 'Invoice Status', 'Invoice Balance Due', 'Amount Applied'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Invoice' || column === 'Invoice Status' ? 'text-left' : 'text-right'}`}>
                            {column}
                          </th>
                        ))}
                      </tr>
                      ...
                      {viewDetail.applications.length > 0 ? (
                        viewDetail.applications.map((application) => (
                          <tr key={application.id}>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{application.invoiceStatusLabel}</td>
                        ...
                        <tr>
                          <td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No applications recorded.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 px-5 py-4">
                  <div>
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Applications</h4>
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Allocate this receipt to posted or partially paid invoices for the selected customer.</p>
                  </div>
                  ...
                  {formState.applications.map((application, index) => (
                    <div key={application.id} className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                      <div className="mb-4 flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => handleRemoveApplication(application.id)} disabled={formState.applications.length === 1} className="rounded-lg px-2 py-1 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 disabled:cursor-not-allowed disabled:opacity-40">
                          Remove
                        </button>
                      </div>
                      <div className="grid gap-4 md:grid-cols-2">
      ...
      <SlideOver isOpen={Boolean(postTarget)} onClose={() => setPostTarget(null)} title="Post Payment Receipt" description="Posting creates the journal entry and locks direct edits on the receipt." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] pt-4">
            <button type="button" onClick={() => setPostTarget(null)} disabled={isPosting} className="rounded-lg bg-gray-100 dark:bg-[var(--card-background)] px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
            <button type="button" onClick={handleConfirmPost} disabled={isPosting} className="rounded-lg bg-blue-600 dark:bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800 disabled:opacity-50">
