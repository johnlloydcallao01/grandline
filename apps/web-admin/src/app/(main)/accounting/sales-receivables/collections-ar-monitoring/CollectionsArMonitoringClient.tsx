...
function SlideOver({
  ...
  return createPortal(
    <div className={`fixed inset-0 z-50 flex justify-end transition-opacity duration-300 ease-in-out ${animate ? 'bg-black/50' : 'bg-transparent'}`} onClick={onClose}>
      <div className={`flex w-full max-w-4xl flex-col bg-white shadow-xl transition-all duration-300 ease-in-out ${animate ? 'translate-x-0' : 'translate-x-full'}`} onClick={(event) => event.stopPropagation()}>
                          ...
                          <tr key={row.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" disabled className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-40" title="Edit invoice is unavailable because overdue queue rows are already posted or partially paid">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" disabled className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title="Delete invoice is unavailable because overdue queue rows are already posted or partially paid">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {viewDetail.lineItems.length > 0 ? viewDetail.lineItems.map((line) => (
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500">No invoice lines found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                          ...
                          <tr key={row.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" disabled className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-40" title="Edit invoice is unavailable because aging rows are already posted or partially paid">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" disabled className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title="Delete invoice is unavailable because aging rows are already posted or partially paid">
                                  <Trash2 className="h-4 w-4" />
                                </button>
                              </div>
                            </td>
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">Collections & AR Monitoring</h1>
          <p className="mt-1 text-base text-gray-600">Monitor overdue receivables and AR aging so customer balances and collection priorities stay visible.</p>
        </div>
      </div>
