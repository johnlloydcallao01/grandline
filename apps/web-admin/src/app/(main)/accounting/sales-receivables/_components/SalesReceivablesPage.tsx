...
type ReceivablesBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type ReceivablesTableCell =
  | string
  | {
      text: string;
      tone?: ReceivablesBadgeTone;
      emphasis?: boolean;
...
type ReceivablesTableRow = {
  id: string;
  cells: ReceivablesTableCell[];
};

export type SalesReceivablesTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: ReceivablesMetric[];
  actions: ReceivablesAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: ReceivablesTableRow[];
};

type SalesReceivablesPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: SalesReceivablesTab[];
  headerActions?: ReceivablesAction[];
};

function getActionIcon(icon?: ReceivablesAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: ReceivablesAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: ReceivablesBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: ReceivablesTableCell, index: number) {
...
function MetricCard({ metric }: { metric: ReceivablesMetric }) {
