...
import { getProjectTasks, getProjectTaskDetail, createProjectTask, updateProjectTask, deleteProjectTask, type ProjectTasksResponse, type ProjectTaskDetail, type PmMetric } from './actions';

type FilterState = { statuses: string[]; billable: string[] };
type TaskForm = { taskCode: string; name: string; projectId: string; status: string; assignedToId: string; billable: boolean; startDate: string; dueDate: string; notes: string };
...
function makeEmptyForm(): TaskForm { return { taskCode: '', name: '', projectId: '', status: 'draft', assignedToId: '', billable: true, startDate: '', dueDate: '', notes: '' }; }
function buildForm(d: ProjectTaskDetail): TaskForm { return { taskCode: d.taskCode || '', name: d.name || '', projectId: d.projectId || '', status: d.status || 'draft', assignedToId: d.assignedToId || '', billable: d.billable, startDate: d.startDate ? d.startDate.slice(0, 10) : '', dueDate: d.dueDate ? d.dueDate.slice(0, 10) : '', notes: d.notes || '' }; }

export function ProjectTasksClient() {
  const [data, setData] = useState<ProjectTasksResponse | null>(null);
  ...
  const [filters, setFilters] = useState<FilterState>({ statuses: [], billable: [] });
  const [quickFilters, setQuickFilters] = useState<string[]>([]);
  const [draftFilters, setDraftFilters] = useState<FilterState>({ statuses: [], billable: [] });
  const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
  const fc = filters.statuses.length + filters.billable.length;
  ...
  const [form, setForm] = useState<TaskForm>(makeEmptyForm());

  const [viewDetail, setViewDetail] = useState<ProjectTaskDetail | null>(null); const [isViewOpen, setIsViewOpen] = useState(false); const [isViewLoading, setIsViewLoading] = useState(false);
  ...
  const fetchData = useCallback(async ({ s, p, nf, nq }: { s: string; p: number; nf: FilterState; nq: string[] }) => { setIsLoading(true); setError(null); try { const r = await getProjectTasks({ search: s, page: p, statuses: nf.statuses, billable: nf.billable, quickFilters: nq }); setData(r); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to load project tasks.'); } finally { setIsLoading(false); } }, []);
  useEffect(() => { void fetchData({ s: submittedSearch, p: currentPage, nf: filters, nq: quickFilters }); }, [filters, currentPage, quickFilters, submittedSearch, fetchData]);

  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); setSubmittedSearch(searchInput); setCurrentPage(1); fetchData({ s: searchInput, p: 1, nf: filters, nq: quickFilters }); };
  const handleRefresh = () => { fetchData({ s: submittedSearch, p: currentPage, nf: filters, nq: quickFilters }); };
  const handleToggleQuickFilter = (v: string) => { setQuickFilters((p) => toggleFilterValue(p, v)); setCurrentPage(1); };
  const handleExport = () => { const rows = data?.section.table.rows; if (!rows?.length) return; const h = ['Task Code', 'Task Name', 'Project', 'Assigned To', 'Billable', 'Status']; const c = [h.map(escapeCsvValue), ...rows.map((r) => [r.taskCode, r.name, r.projectLabel, r.assigneeLabel, r.billableLabel, r.statusLabel].map(escapeCsvValue))].map((r) => r.join(',')).join('\n'); const b = new Blob([c], { type: 'text/csv;charset=utf-8;' }); const u = URL.createObjectURL(b); const l = document.createElement('a'); l.href = u; l.download = 'project-tasks.csv'; document.body.appendChild(l); l.click(); document.body.removeChild(l); URL.revokeObjectURL(u); };

  const handleOpenCreate = () => { setForm(makeEmptyForm()); setCreateErr(null); setIsCreateOpen(true); };
  const handleCreateSubmit = async (e: React.FormEvent) => { e.preventDefault(); setCreateErr(null); setIsCreateSubmitting(true); try { await createProjectTask({ taskCode: form.taskCode || undefined, name: form.name, projectId: form.projectId, status: form.status, assignedToId: form.assignedToId || undefined, billable: form.billable, startDate: form.startDate || undefined, dueDate: form.dueDate || undefined, notes: form.notes || undefined }); setIsCreateOpen(false); handleRefresh(); } catch (err) { setCreateErr(err instanceof Error ? err.message : 'An error occurred'); } finally { setIsCreateSubmitting(false); } };

  const handleView = async (id: string) => { setIsViewOpen(true); setIsViewLoading(true); setViewDetail(null); try { setViewDetail(await getProjectTaskDetail(id)); } catch (e) { console.error(e); } finally { setIsViewLoading(false); } };
  const handleOpenEdit = async (id: string) => { setIsEditOpen(true); setEditErr(null); setEditId(id); setIsViewLoading(true); setViewDetail(null); try { const d = await getProjectTaskDetail(id); setViewDetail(d); setForm(buildForm(d)); } catch (e) { setEditErr(e instanceof Error ? e.message : 'Unable to load details.'); } finally { setIsViewLoading(false); } };
  const handleEditSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!editId) return; setEditErr(null); setIsEditSubmitting(true); try { await updateProjectTask(editId, { taskCode: form.taskCode || undefined, name: form.name, projectId: form.projectId, status: form.status, assignedToId: form.assignedToId || undefined, billable: form.billable, startDate: form.startDate || undefined, dueDate: form.dueDate || undefined, notes: form.notes || undefined }); setIsEditOpen(false); handleRefresh(); } catch (err) { setEditErr(err instanceof Error ? err.message : 'An error occurred'); } finally { setIsEditSubmitting(false); } };
  ...
  const handleConfirmDelete = async () => { if (!deleteId) return; setIsDeleteSubmitting(true); setDeleteErr(null); try { await deleteProjectTask(deleteId); setIsDeleteOpen(false); handleRefresh(); } catch (err) { setDeleteErr(err instanceof Error ? err.message : 'Unable to delete.'); } finally { setIsDeleteSubmitting(false); } };

  const rd = data?.referenceData;
  const projectOpts = [{ label: 'Select a project', value: '' }, ...(rd?.projects || []).map((p) => ({ label: p.label, value: p.id }))];
  const userOpts = [{ label: 'None', value: '' }, ...(rd?.users || []).map((u) => ({ label: u.label, value: u.id }))];
  const statusOpts = rd?.statusOptions || [];

  const renderForm = () => (<>
    <div className="grid grid-cols-2 gap-4">
      <FormField label="Task Code"><Input value={form.taskCode} onChange={(v) => setForm({ ...form, taskCode: v })} placeholder="Auto-generated if empty" /></FormField>
      <FormField label="Status" required><Select value={form.status} onChange={(v) => setForm({ ...form, status: v })} options={statusOpts} /></FormField>
    </div>
    <FormField label="Task Name" required><Input value={form.name} onChange={(v) => setForm({ ...form, name: v })} placeholder="Task name" required /></FormField>
    <FormField label="Project" required><Select value={form.projectId} onChange={(v) => setForm({ ...form, projectId: v })} options={projectOpts} /></FormField>
    <div className="grid grid-cols-2 gap-4">
      <FormField label="Assigned To"><Select value={form.assignedToId} onChange={(v) => setForm({ ...form, assignedToId: v })} options={userOpts} /></FormField>
      <FormField label="Billable" required><Select value={form.billable ? 'true' : 'false'} onChange={(v) => setForm({ ...form, billable: v === 'true' })} options={[{ label: 'Yes', value: 'true' }, { label: 'No', value: 'false' }]} /></FormField>
    </div>
    <div className="grid grid-cols-2 gap-4">
      <FormField label="Start Date"><Input type="date" value={form.startDate} onChange={(v) => setForm({ ...form, startDate: v })} /></FormField>
      <FormField label="Due Date"><Input type="date" value={form.dueDate} onChange={(v) => setForm({ ...form, dueDate: v })} /></FormField>
    </div>
  ...
  return (
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1"><h2 className="text-lg font-semibold text-gray-900">Project Tasks</h2><p className="text-sm text-gray-600">Review project work units using task code, project, assigned user, billable flag, task status, and due-date tracking.</p><p className="text-sm text-gray-500">{data?.totals.filteredRows ?? 0} matching rows</p></div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={handleRefresh} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('secondary')}`}><RefreshCw className="h-4 w-4" /> Refresh Tasks</button>
          <button type="button" onClick={handleOpenCreate} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('primary')}`}><Plus className="h-4 w-4" /> New Task</button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input type="text" placeholder={data?.section.searchPlaceholder || 'Search tasks'} value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></div><button type="submit" className="inline-flex items-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:border-blue-700 hover:bg-blue-700"><Search className="h-4 w-4" /> Search</button></form>
            <button type="button" onClick={() => { if (!isFilterPanelOpen) setDraftFilters({ ...filters }); setIsFilterPanelOpen((p) => !p); }} className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${isFilterPanelOpen || fc > 0 ? 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}><Filter className="h-4 w-4" /> Filters{fc > 0 && <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-blue-600 px-1.5 py-0.5 text-[11px] font-semibold text-white">{fc}</span>}</button>
          </div>
      ...
      <SlideOver isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="New Task" description="Create a new project task."><form onSubmit={handleCreateSubmit} className="space-y-6">{createErr && <div className="flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700"><AlertCircle className="h-4 w-4 shrink-0" />{createErr}</div>}{renderForm()}<div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4"><button type="button" onClick={() => setIsCreateOpen(false)} disabled={isCreateSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50">Cancel</button><button type="submit" disabled={isCreateSubmitting} className={`rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors disabled:opacity-50 ${getActionClasses('primary')}`}>{isCreateSubmitting ? 'Creating...' : 'Create Task'}</button></div></form></SlideOver>

      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Task Detail" description="View full task record.">
        <div className="space-y-6">{isViewLoading ? <LoadingSkeleton /> : viewDetail ? (<div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
          <div className="flex justify-between border-b pb-3"><span className="text-sm text-gray-500">Task Code</span><span className="text-sm font-medium text-gray-900">{viewDetail.taskCode}</span></div>
          ...
          <div className="flex justify-between border-b pb-3"><span className="text-sm text-gray-500">Project</span><span className="text-sm font-medium text-gray-900">{viewDetail.projectLabel}</span></div>
          ...
          <div className="flex justify-between border-b pb-3"><span className="text-sm text-gray-500">Assigned To</span><span className="text-sm font-medium text-gray-900">{viewDetail.assigneeLabel}</span></div>
          <div className="flex justify-between border-b pb-3"><span className="text-sm text-gray-500">Billable</span><span className="text-sm font-medium text-gray-900">{viewDetail.billable ? 'Yes' : 'No'}</span></div>
          ...
          <div className="flex justify-between border-b pb-3"><span className="text-sm text-gray-500">Due Date</span><span className="text-sm font-medium text-gray-900">{viewDetail.dueDateLabel}</span></div>
      ...
      <SlideOver isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Task" description="Modify the project task."><form onSubmit={handleEditSubmit} className="space-y-6">{isViewLoading ? <LoadingSkeleton /> : <>{editErr && <div className="flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700"><AlertCircle className="h-4 w-4 shrink-0" />{editErr}</div>}{renderForm()}<div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4"><button type="button" onClick={() => setIsEditOpen(false)} disabled={isEditSubmitting} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50">Cancel</button><button type="submit" disabled={isEditSubmitting} className={`rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors disabled:opacity-50 ${getActionClasses('primary')}`}>{isEditSubmitting ? 'Saving...' : 'Save Changes'}</button></div></>}</form></SlideOver>

      <SlideOver isOpen={isDeleteOpen} onClose={handleCloseDelete} title="Delete Task" description="Remove this task permanently.">
        <div className="space-y-6">{deleteErr && <div className="flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700"><AlertCircle className="h-4 w-4 shrink-0" />{deleteErr}</div>}<div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><p className="font-medium">Are you sure?</p><p className="mt-1">Task &quot;{deleteCode}&quot; will be permanently removed.</p></div><div className="flex justify-end gap-3 border-t border-gray-200 pt-4"><button type="button" onClick={handleCloseDelete} disabled={isDeleteSubmitting} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 disabled:opacity-50">Cancel</button><button type="button" onClick={handleConfirmDelete} disabled={isDeleteSubmitting} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50">{isDeleteSubmitting ? 'Deleting...' : 'Delete Task'}</button></div></div>
      </SlideOver>
    </div>
  );
}
