...
type ProjectsTimeAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type ProjectsTimeMetric = {
  label: string;
  value: string;
  change: string;
...
type ProjectsTimeBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type ProjectsTimeTableCell =
  | string
  | {
      text: string;
      tone?: ProjectsTimeBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type ProjectsTimeTableRow = {
  id: string;
  cells: ProjectsTimeTableCell[];
};

export type ProjectsTimeTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: ProjectsTimeMetric[];
  actions: ProjectsTimeAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: ProjectsTimeTableRow[];
};

type ProjectsTimePageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: ProjectsTimeTab[];
  headerActions?: ProjectsTimeAction[];
};

function getActionIcon(icon?: ProjectsTimeAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: ProjectsTimeAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: ProjectsTimeBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: ProjectsTimeTableCell, index: number) {
...
function MetricCard({ metric }: { metric: ProjectsTimeMetric }) {
...
export function ProjectsTimePage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: ProjectsTimePageProps) {
