...
type PayrollBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type PayrollTableCell =
  | string
  | {
      text: string;
...
type PayrollTableRow = {
  id: string;
  cells: PayrollTableCell[];
};

export type PayrollContractorFinanceTab = {
  id: string;
  label: string;
  description: string;
...
type PayrollContractorFinancePageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: PayrollContractorFinanceTab[];
  headerActions?: PayrollAction[];
};

function getActionIcon(icon?: PayrollAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: PayrollAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: PayrollBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: PayrollTableCell, index: number) {
...
function MetricCard({ metric }: { metric: PayrollMetric }) {
...
export function PayrollContractorFinancePage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: PayrollContractorFinancePageProps) {
