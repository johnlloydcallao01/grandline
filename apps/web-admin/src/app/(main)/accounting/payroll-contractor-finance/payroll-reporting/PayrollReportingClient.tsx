    ...
    return (
      <td className="px-4 py-3 text-right">
        <div className="flex justify-end gap-2">
          ...
          <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:cursor-not-allowed disabled:opacity-40" title="Edit payroll run">
            <Edit className="h-4 w-4" />
          </button>
          <button type="button" onClick={() => setPostTarget({ id: row.id, payrollCode: row.payrollCode })} disabled={!canPost} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 dark:hover:text-blue-300 disabled:cursor-not-allowed disabled:opacity-40" title={canPost ? 'Post payroll run' : 'Payroll run must be approved with entries before posting'}>
            <Send className="h-4 w-4" />
          </button>
          <button type="button" onClick={() => setDeleteTarget({ id: row.id, payrollCode: row.payrollCode })} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 dark:hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-40" title="Delete payroll run">
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">Payroll Reporting</h1>
          <p className="mt-1 text-base text-gray-600 dark:text-gray-400">Review payroll posting summaries, run status, deduction rollups, net payable totals, and journal linkage before and after General Ledger posting.</p>
        </div>
      </div>
      ...
      <div className="border-b border-gray-200 dark:border-gray-800">
        <nav className="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs">
          {STATIC_TABS.map((tab) => {
            ...
            return (
              <button key={tab.id} type="button" onClick={() => handleTabChange(tab.id)} className={`whitespace-nowrap border-b-2 px-1 py-4 text-sm font-medium ${isActive ? 'border-blue-600 text-blue-600 dark:border-blue-700 dark:text-blue-400' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700 dark:text-gray-400 dark:hover:border-gray-600 dark:hover:text-gray-300'}`}>
                {tab.label}
              </button>
            );
          })}
      ...
      {error ? (
        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-950/30 p-3 text-sm text-red-700 dark:text-red-400">
          ...
          <button type="button" onClick={() => setError(null)} className="rounded-lg p-1 text-red-500 hover:bg-red-100 dark:text-red-400 dark:hover:bg-red-950/30"><X className="h-4 w-4" /></button>
        </div>
      ) : null}
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{reportData?.totals.filteredRows ?? 0} matching rows</p>
        </div>
            ...
            <div className="space-y-4 p-5">
                ...
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Select statuses and posting states to narrow the report.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <button type="button" onClick={handleClearFilters} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">Clear all</button>
                      ...
                      <button type="button" onClick={handleApplyFilters} className="rounded-lg border border-blue-600 dark:border-blue-700 bg-blue-600 dark:bg-blue-700 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800">Apply Filters</button>
                    </div>
                  </div>
                  <div className="mt-6 grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                      <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Posting State</h5>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {(reportData?.filterOptions.postingStates || []).map((option) => {
                          const selected = draftFilters.postingStates.includes(option.value);
                          return <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, postingStates: toggleFilterValue(previous.postingStates, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 dark:bg-blue-700 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{option.label}</button>;
                        })}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{reportData?.meta.tableTitle || currentTab.tableTitle}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{reportData?.meta.tableDescription || currentTab.tableDescription}</p>
                </div>
                ...
                <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        <thead className="bg-gray-50 dark:bg-gray-800/50">
                          <tr>
                            {(reportData?.meta.columns || currentTab.columns).map((column) => (
                              <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${['Gross Amount', 'Deduction Amount', 'Net Amount'].includes(column) ? 'text-right' : 'text-left'}`}>{column}</th>
                            ))}
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-[var(--card-background)]">
                          {(reportData?.rows || []).length > 0 ? (
                            ...
                            <tr>
                              <td colSpan={(reportData?.meta.columns || currentTab.columns).length + 1} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {reportData?.pagination && reportData.pagination.totalPages > 1 ? (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-600 dark:text-gray-400">Page {reportData.pagination.page} of {reportData.pagination.totalPages}</p>
                      <div className="flex gap-2">
                        <button type="button" disabled={!reportData.pagination.hasPrevPage} onClick={() => setCurrentPage((previous) => Math.max(1, previous - 1))} className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50">
                          Previous
                        </button>
                        <button type="button" disabled={!reportData.pagination.hasNextPage} onClick={() => setCurrentPage((previous) => previous + 1)} className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50">
                          Next
                        </button>
                      </div>
                    </div>
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
                <div className="border-b border-gray-200 dark:border-gray-800 px-5 py-4">
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Payroll Entries</h4>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-[var(--card-background)]">
                      {entries.length > 0 ? entries.map((entry) => (
                        <tr key={String(entry.id)}>
                          <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{buildPersonName(entry)}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{String(entry.entryType || '-')}</td>
                          <td className="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400">{new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(Number(entry.grossAmount || 0))}</td>
                          <td className="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400">{new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(Number(entry.deductionAmount || 0))}</td>
                          <td className="px-4 py-3 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">{new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(Number(entry.netAmount || 0))}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{buildName(entry.expenseAccount) || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{buildName(entry.payableAccount) || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{String(entry.status || '-')}</td>
                        </tr>
                      )) : (
                        <tr>
                          <td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No payroll entries linked to this run.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Payroll Run" description="Delete this mutable payroll run after dependency validation completes." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Payroll Run'}</button>
          </div>
        </div>
      </SlideOver>
