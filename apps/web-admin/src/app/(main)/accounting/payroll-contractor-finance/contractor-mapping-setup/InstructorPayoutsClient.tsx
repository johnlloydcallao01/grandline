...
import {
  ...
  deleteInstructorPayout,
  getInstructorPayoutDetail,
  getInstructorPayouts,
  updateInstructorPayout,
  type InstructorPayoutDetail,
  type InstructorPayoutMutationInput,
  type InstructorPayoutsCell,
  type InstructorPayoutsMetric,
  type InstructorPayoutsRegisterResponse,
} from './actions';

type InstructorPayoutFilterState = { statuses: string[] };
type InstructorPayoutFormState = {
  instructor: string;
  course: string;
  periodStart: string;
  periodEnd: string;
...
type InstructorPayoutActionTarget = {
  id: string;
  name: string;
};

const MUTABLE_STATUSES = new Set(['draft', 'calculated']);
...
function getMetricTone(trend: InstructorPayoutsMetric['trend']) {
...
function createEmptyPayoutForm(): InstructorPayoutFormState {
...
function renderCell(cell: InstructorPayoutsCell, index: number) {
...
export function InstructorPayoutsClient() {
  const [data, setData] = useState<InstructorPayoutsRegisterResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [searchInput, setSearchInput] = useState('');
  const [submittedSearch, setSubmittedSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [filters, setFilters] = useState<InstructorPayoutFilterState>({ statuses: [] });
  const [draftFilters, setDraftFilters] = useState<InstructorPayoutFilterState>({ statuses: [] });
  const [quickFilters, setQuickFilters] = useState<string[]>([]);
  const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
  const [viewDetail, setViewDetail] = useState<InstructorPayoutDetail | null>(null);
  const [isViewOpen, setIsViewOpen] = useState(false);
  const [isViewLoading, setIsViewLoading] = useState(false);
  const [formState, setFormState] = useState<InstructorPayoutFormState>(createEmptyPayoutForm());
  const [editingId, setEditingId] = useState<string | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<InstructorPayoutActionTarget | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);
  const [approveTarget, setApproveTarget] = useState<InstructorPayoutActionTarget | null>(null);
  ...
  const fetchPayouts = useCallback(async ({
    ...
    search: string;
    page: number;
    nextFilters: InstructorPayoutFilterState;
    nextQuickFilters: string[];
  }) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await getInstructorPayouts({
        search,
        page,
      ...
      setError(fetchError instanceof Error ? fetchError.message : 'Unable to load instructor payouts.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  }, [currentPage, fetchPayouts, filters, quickFilters, submittedSearch]);
  ...
  const instructorOptions = useMemo(
    () => [
      ...
      ...(referenceData?.instructors || []).map((instructor) => ({
  ...
  const handleSearch = (event: React.FormEvent) => {
    ...
    setCurrentPage(1);
    void fetchPayouts({ search: searchInput, page: 1, nextFilters: filters, nextQuickFilters: quickFilters });
  };

  const handleRefresh = () => {
    void fetchPayouts({ search: submittedSearch, page: currentPage, nextFilters: filters, nextQuickFilters: quickFilters });
  };
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = 'instructor-payouts.csv';
  ...
  const handleView = async (id: string) => {
    ...
    setViewDetail(null);
    try {
      const detail = await getInstructorPayoutDetail(id);
  ...
  const handleOpenEdit = async (id: string) => {
    ...
    try {
      ...
      setFormState({
        ...
        sourceType: detail.sourceType || 'course_activity',
        ...
        status: detail.status === 'calculated' ? 'calculated' : 'draft',
        notes: detail.notes || '',
      });
    } catch (detailError) {
  ...
  const normalizeFormPayload = (): InstructorPayoutMutationInput => ({
    instructor: formState.instructor,
    course: formState.course,
    periodStart: formState.periodStart,
    periodEnd: formState.periodEnd,
    sourceType: formState.sourceType.trim() || 'course_activity',
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateInstructorPayout(editingId, payload);
      } else {
        await createInstructorPayout(payload);
      }
      setIsFormOpen(false);
  ...
  const handleConfirmDelete = async () => {
    ...
    setError(null);
    try {
      await deleteInstructorPayout(deleteTarget.id);
  ...
  const handleConfirmApprove = async () => {
    ...
    setError(null);
    try {
      await approveInstructorPayout(approveTarget.id);
      setApproveTarget(null);
      await refreshCurrentView();
    } catch (approveError) {
      setError(approveError instanceof Error ? approveError.message : 'Unable to approve payout.');
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Instructor Payouts</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">Review payout obligations and approvals for instructors using source reference, course, period dates, calculated amount, approved amount, and payout status.</p>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder={data?.meta?.searchPlaceholder || 'Search instructor, course, period, source reference, approved amount, or payout status'} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 dark:focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta?.tableTitle || 'Instructor Payout Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta?.tableDescription || 'Payout rows aligned to accounting-instructor-payouts, which is the actual contractor-adjacent payout surface in the backend today.'}</p>
            </div>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-[var(--card-border)]">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Instructor', 'Course', 'Period', 'Calculated Amount', 'Approved Amount', 'Status'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Calculated Amount' || column === 'Approved Amount' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-[var(--card-background)]">
                      {(data?.rows || []).length > 0 ? (data?.rows || []).map((row) => {
                        ...
                        const canApprove = row.status === 'calculated';
                        return (
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:cursor-not-allowed disabled:opacity-40" title="Edit payout">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setApproveTarget({ id: row.id, name: row.instructorName })} disabled={!canApprove} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 dark:text-blue-400 transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-700 dark:hover:text-blue-300 disabled:cursor-not-allowed disabled:opacity-40" title="Approve payout">
                                  <CheckCircle className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setDeleteTarget({ id: row.id, name: row.instructorName })} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 dark:hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-40" title="Delete payout">
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No instructor payout rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Payout Detail" description="Review payout header values, period, amounts, and status.">
        <div className="space-y-6">
          {isViewLoading ? <LoadingSkeleton /> : viewDetail ? (
            <>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  ['Instructor', viewDetail.instructorName || viewDetail.instructor || '-'],
                  ['Course', viewDetail.courseName || viewDetail.course || '-'],
                  ['Period Start', viewDetail.periodStart ? viewDetail.periodStart.slice(0, 10) : '-'],
                  ['Period End', viewDetail.periodEnd ? viewDetail.periodEnd.slice(0, 10) : '-'],
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-2">
                {[
                  ['Calculated Amount', formatCurrency(viewDetail.calculatedAmount || 0)],
                  ['Approved Amount', formatCurrency(viewDetail.approvedAmount || 0)],
                ].map(([label, value]) => (
      ...
      <SlideOver isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingId ? 'Edit Payout' : 'Create Payout'} description="Set instructor, course, period, calculated amount, and optional approved amount.">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <FormField label="Status" required>
              <Select value={formState.status} onChange={(value) => setFormState((previous) => ({ ...previous, status: value }))} options={[{ label: 'Draft', value: 'draft' }, { label: 'Calculated', value: 'calculated' }]} />
            </FormField>
            <FormField label="Source Type">
              <Select value={formState.sourceType} onChange={(value) => setFormState((previous) => ({ ...previous, sourceType: value }))} options={[{ label: 'Course Activity', value: 'course_activity' }, { label: 'Manual', value: 'manual' }, { label: 'Adjustment', value: 'adjustment' }]} />
            </FormField>
            ...
            <FormField label="Calculated Amount" required>
            ...
            <FormField label="Approved Amount">
              <Input type="number" value={formState.approvedAmount} onChange={(value) => setFormState((previous) => ({ ...previous, approvedAmount: value }))} placeholder="Leave blank if not yet approved" />
            </FormField>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="submit" disabled={isSubmitting} className={`rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors disabled:opacity-50 ${getActionClasses('primary')}`}>{isSubmitting ? 'Saving...' : editingId ? 'Save Changes' : 'Create Payout'}</button>
          </div>
        </form>
      </SlideOver>

      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Payout" description="Delete this mutable payout. Only draft and calculated payouts can be deleted." width="max-w-lg">
        <div className="space-y-6">
          <div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-4 text-sm text-red-700 dark:text-red-400">
            <p className="font-medium">Delete payout for {deleteTarget?.name}?</p>
            <p className="mt-1">Draft and calculated payouts can be deleted. Approved or paid payouts cannot be deleted.</p>
          </div>
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Payout'}</button>
          </div>
        </div>
      </SlideOver>

      <SlideOver isOpen={Boolean(approveTarget)} onClose={() => setApproveTarget(null)} title="Approve Payout" description="Approving finalizes the payout amount. Only calculated payouts can be approved." width="max-w-lg">
        <div className="space-y-6">
          <div className="rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30 p-4 text-sm text-blue-700 dark:text-blue-400">
            <p className="font-medium">Approve payout for {approveTarget?.name}?</p>
            <p className="mt-1">Make sure the instructor, course, and calculated amount are correct before approving.</p>
          </div>
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            <button type="button" onClick={() => setApproveTarget(null)} disabled={isApproving} className="rounded-lg bg-gray-100 dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50">Cancel</button>
            <button type="button" onClick={handleConfirmApprove} disabled={isApproving} className="rounded-lg bg-blue-600 dark:bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-800 disabled:opacity-50">{isApproving ? 'Approving...' : 'Approve Payout'}</button>
          </div>
        </div>
      </SlideOver>
    </div>
