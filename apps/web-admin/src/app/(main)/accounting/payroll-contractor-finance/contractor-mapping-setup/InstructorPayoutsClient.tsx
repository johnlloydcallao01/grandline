    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-gray-600">Review payout obligations and approvals for instructors using source reference, course, period dates, calculated amount, approved amount, and payout status.</p>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder={data?.meta?.searchPlaceholder || 'Search instructor, course, period, source reference, approved amount, or payout status'} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900">{data?.meta?.tableTitle || 'Instructor Payout Register'}</h3>
              <p className="text-sm text-gray-600">{data?.meta?.tableDescription || 'Payout rows aligned to accounting-instructor-payouts, which is the actual contractor-adjacent payout surface in the backend today.'}</p>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        {['Instructor', 'Course', 'Period', 'Calculated Amount', 'Approved Amount', 'Status'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 ${column === 'Calculated Amount' || column === 'Approved Amount' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {(data?.rows || []).length > 0 ? (data?.rows || []).map((row) => {
                          ...
                          <tr key={row.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-40" title="Edit payout">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setApproveTarget({ id: row.id, name: row.instructorName })} disabled={!canApprove} className="inline-flex items-center gap-1 rounded-lg p-2 text-blue-600 transition-colors hover:bg-blue-50 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-40" title="Approve payout">
                                  <CheckCircle className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setDeleteTarget({ id: row.id, name: row.instructorName })} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-40" title="Delete payout">
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500">No instructor payout rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Payout" description="Delete this mutable payout. Only draft and calculated payouts can be deleted." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Payout'}</button>
          </div>
        </div>
      </SlideOver>
      ...
      <SlideOver isOpen={Boolean(approveTarget)} onClose={() => setApproveTarget(null)} title="Approve Payout" description="Approving finalizes the payout amount. Only calculated payouts can be approved." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4">
            <button type="button" onClick={() => setApproveTarget(null)} disabled={isApproving} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 disabled:opacity-50">Cancel</button>
            <button type="button" onClick={handleConfirmApprove} disabled={isApproving} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">{isApproving ? 'Approving...' : 'Approve Payout'}</button>
          </div>
        </div>
      </SlideOver>
    </div>
