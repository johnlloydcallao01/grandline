...
type LmsMonetizationAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type LmsMonetizationMetric = {
  label: string;
  value: string;
  change: string;
...
type LmsMonetizationBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type LmsMonetizationTableCell =
  | string
  | {
      text: string;
      tone?: LmsMonetizationBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type LmsMonetizationTableRow = {
  id: string;
  cells: LmsMonetizationTableCell[];
};

export type LmsMonetizationTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: LmsMonetizationMetric[];
  actions: LmsMonetizationAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: LmsMonetizationTableRow[];
};

type LmsMonetizationSetupPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: LmsMonetizationTab[];
  headerActions?: LmsMonetizationAction[];
};

function getActionIcon(icon?: LmsMonetizationAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: LmsMonetizationAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: LmsMonetizationBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: LmsMonetizationTableCell, index: number) {
...
function MetricCard({ metric }: { metric: LmsMonetizationMetric }) {
...
export function LmsMonetizationSetupPage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: LmsMonetizationSetupPageProps) {
