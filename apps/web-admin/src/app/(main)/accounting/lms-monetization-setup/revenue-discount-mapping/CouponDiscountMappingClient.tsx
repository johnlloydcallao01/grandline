  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={disabled}
        className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-3 py-2 text-sm text-left text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 dark:focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50 disabled:bg-gray-50 disabled:text-gray-500"
      >
        {value.length > 0
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full rounded-lg border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-lg max-h-60 overflow-y-auto">
          {options.length === 0 ? (
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Coupon & Discount Mapping</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">Review coupon-code definitions and the discount impact they create through enrollment billing link snapshots used in LMS finance reporting.</p>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder={data?.meta.searchPlaceholder || 'Search coupon code, name, status, discount type, amount, or scope'} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 dark:focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          {isFilterPanelOpen ? (
            <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters({ statuses: [], discountTypes: [] }); setFilters({ statuses: [], discountTypes: [] }); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">Clear all</button>
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-2">
                ...
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Discount Type</h5>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(data?.filterOptions.discountTypes || []).map((option) => {
                      const selected = draftFilters.discountTypes.includes(option.value);
                      return <button key={option.value} type="button" onClick={() => setDraftFilters((previous) => ({ ...previous, discountTypes: toggleFilterValue(previous.discountTypes, option.value) }))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selected ? 'bg-blue-600 dark:bg-blue-700 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{option.label}</button>;
                    })}
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta.tableTitle || 'Coupon And Discount Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta.tableDescription || 'Coupon and discount view aligned to `coupon-codes` plus usage tracking from coupon redemptions.'}</p>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Coupon Code', 'Status', 'Discount Type', 'Amount', 'Scope', 'Usage Count'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Amount' || column === 'Usage Count' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-[var(--card-background)]">
                      {(data?.rows || []).length > 0 ? (data?.rows || []).map((row) => {
                          ...
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" onClick={() => handleOpenEdit(row.id)} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-amber-500 dark:text-amber-400 transition-colors hover:bg-amber-50 dark:hover:bg-amber-950/30 hover:text-amber-700 dark:hover:text-amber-300 disabled:cursor-not-allowed disabled:opacity-40" title="Edit coupon">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setDeleteTarget({ id: row.id, code: row.code })} disabled={!isMutable} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 dark:hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-40" title="Delete coupon">
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No coupon code rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          {isViewLoading ? <LoadingSkeleton /> : viewDetail ? (
            <>
              ...
              {viewDetail.includedCourseIds.length > 0 ? (
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
                  <div className="border-b border-gray-200 dark:border-[var(--card-border)] px-5 py-4">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Included Courses</h4>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                      <thead className="bg-gray-50 dark:bg-gray-800/50">
              ...
              {viewDetail.includedCategoryIds.length > 0 ? (
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
                  <div className="border-b border-gray-200 dark:border-[var(--card-border)] px-5 py-4">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Included Categories</h4>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                      <thead className="bg-gray-50 dark:bg-gray-800/50">
      ...
      <SlideOver isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingId ? 'Edit Coupon' : 'New Coupon'} description="Configure coupon code, discount rules, scope, usage limits, and scheduling for enrollment checkout.">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Discount Configuration</h4>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Scope & Targeting</h4>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Scheduling & Limits</h4>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Restrictions</h4>
            <div className="grid gap-4 md:grid-cols-2">
      ...
      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Coupon" description="Delete this coupon code." width="max-w-lg">
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Coupon'}</button>
          </div>
        </div>
      </SlideOver>
    </div>
