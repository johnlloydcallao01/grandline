...
import {
  ...
  type CorporateAccountMutationInput,
  type CorporateAccountsRegisterResponse,
} from './actions-billing-policy';

type CorpAccountsFilterState = { statuses: string[] };
type CorpAccountsFormState = {
  accountCode: string;
  name: string;
  customer: string;
...
function createEmptyForm(): CorpAccountsFormState {
  return {
    accountCode: '',
    name: '',
    customer: '',
...
export function CorporateAccountsClient() {
  const [data, setData] = useState<CorporateAccountsRegisterResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [searchInput, setSearchInput] = useState('');
  const [submittedSearch, setSubmittedSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [filters, setFilters] = useState<CorpAccountsFilterState>({ statuses: [] });
  const [draftFilters, setDraftFilters] = useState<CorpAccountsFilterState>({ statuses: [] });
  const [quickFilters, setQuickFilters] = useState<string[]>([]);
  const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
  const [viewDetail, setViewDetail] = useState<CorporateAccountDetail | null>(null);
  const [isViewOpen, setIsViewOpen] = useState(false);
  const [isViewLoading, setIsViewLoading] = useState(false);
  const [formState, setFormState] = useState<CorpAccountsFormState>(createEmptyForm());
  const [editingId, setEditingId] = useState<string | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<CorpAccountsActionTarget | null>(null);
  ...
  const fetchAccounts = useCallback(async ({
    ...
    search: string;
    page: number;
    nextFilters: CorpAccountsFilterState;
    nextQuickFilters: string[];
  }) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await getCorporateAccounts({
        search,
        page,
  ...
  }, [currentPage, fetchAccounts, filters, quickFilters, submittedSearch]);
  ...
  const handleSearch = (event: React.FormEvent) => {
    ...
    setCurrentPage(1);
    void fetchAccounts({ search: searchInput, page: 1, nextFilters: filters, nextQuickFilters: quickFilters });
  };

  const handleRefresh = () => {
    void fetchAccounts({ search: submittedSearch, page: currentPage, nextFilters: filters, nextQuickFilters: quickFilters });
  };
  ...
  const handleExport = () => {
    ...
    const headers = ['Account Code', 'Name', 'Customer', 'Billing Contact', 'Credit Terms', 'Status'];
    ...
    link.href = url;
    link.download = 'corporate-accounts.csv';
  ...
  const normalizeFormPayload = (): CorporateAccountMutationInput => ({
    accountCode: formState.accountCode.trim() || undefined,
    name: formState.name.trim(),
    customer: formState.customer,
    billingContact: formState.billingContact.trim() || undefined,
    email: formState.email.trim() || undefined,
    phone: formState.phone.trim() || undefined,
    creditTerms: formState.creditTerms.trim() || undefined,
    paymentTerms: formState.paymentTerms.trim() || undefined,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateCorporateAccount(editingId, payload);
      } else {
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Corporate Accounts</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">Manage corporate payer accounts linked to customers with contact data, credit terms, payment terms, and status.</p>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder={data?.meta?.searchPlaceholder || 'Search account code, company name, customer, billing contact, or status'} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 dark:focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.meta?.tableTitle || 'Corporate Account Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.meta?.tableDescription || 'Corporate account records using account code, linked customer, billing contact, terms, and status.'}</p>
            </div>
                    ...
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Account Code', 'Name', 'Customer', 'Billing Contact', 'Credit Terms', 'Status'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Credit Terms' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-[var(--card-background)]">
                      {(data?.rows || []).length > 0 ? (data?.rows || []).map((row) => {
                        ...
                        const cells: BillingPolicyCell[] = [
                          { text: row.accountCode, emphasis: true },
                          row.name,
                          row.customerLabel || '-',
                          row.billingContact || '-',
                          row.creditTerms || '-',
                          ...
                          <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button type="button" onClick={() => handleOpenEdit(row.id)} className="inline-flex items-center gap-1 rounded-lg p-2 text-amber-500 dark:text-amber-400 transition-colors hover:bg-amber-50 dark:hover:bg-amber-950/30 hover:text-amber-700 dark:hover:text-amber-300" title="Edit corporate account">
                                  <Edit className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setDeleteTarget({ id: row.id, name: row.name })} className="inline-flex items-center gap-1 rounded-lg p-2 text-red-500 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 dark:hover:text-red-300" title="Delete corporate account">
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Corporate Account Detail" description="Review corporate account header values, customer mapping, terms, and usage summary.">
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  ...
                  ['Corporate Billing Links', String(viewDetail.usageSummary?.corporateBillingLinkCount ?? 0)],
      ...
      <SlideOver isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingId ? 'Edit Corporate Account' : 'New Corporate Account'} description="Configure corporate payer account including customer mapping, contact details, credit terms, and operational status.">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <FormField label="Account Code">
              <Input value={formState.accountCode} onChange={(value) => setFormState((previous) => ({ ...previous, accountCode: value }))} placeholder="e.g. CORP-0001" />
            </FormField>
            <FormField label="Name" required>
              <Input value={formState.name} onChange={(value) => setFormState((previous) => ({ ...previous, name: value }))} placeholder="Company name" required />
            </FormField>
            ...
            <FormField label="Billing Contact">
              <Input value={formState.billingContact} onChange={(value) => setFormState((previous) => ({ ...previous, billingContact: value }))} placeholder="Full name" />
            </FormField>
            <FormField label="Email">
              <Input type="email" value={formState.email} onChange={(value) => setFormState((previous) => ({ ...previous, email: value }))} placeholder="billing@company.com" />
            </FormField>
            ...
            <FormField label="Credit Terms">
              <Input value={formState.creditTerms} onChange={(value) => setFormState((previous) => ({ ...previous, creditTerms: value }))} placeholder="e.g. 30 days" />
            </FormField>
            <FormField label="Payment Terms">
              <Input value={formState.paymentTerms} onChange={(value) => setFormState((previous) => ({ ...previous, paymentTerms: value }))} placeholder="e.g. Bank transfer" />
            </FormField>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="submit" disabled={isSubmitting} className={`rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors disabled:opacity-50 ${getActionClasses('primary')}`}>{isSubmitting ? 'Saving...' : editingId ? 'Save Changes' : 'Create Corporate Account'}</button>
          </div>
        </form>
      </SlideOver>

      <SlideOver isOpen={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Delete Corporate Account" description="Delete this corporate account record." width="max-w-lg">
        <div className="space-y-6">
          <div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-4 text-sm text-red-700 dark:text-red-400">
            ...
            <p className="mt-1">This will permanently remove the corporate account record. This action cannot be undone if the account has no linked corporate billing links.</p>
          </div>
          <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-4">
            ...
            <button type="button" onClick={handleConfirmDelete} disabled={isDeleting} className="rounded-lg bg-red-600 dark:bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 dark:hover:bg-red-800 disabled:opacity-50">{isDeleting ? 'Deleting...' : 'Delete Corporate Account'}</button>
          </div>
        </div>
      </SlideOver>
    </div>
