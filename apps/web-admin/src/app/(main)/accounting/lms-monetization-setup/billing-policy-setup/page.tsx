...
import { ScholarshipSponsorsClient } from './ScholarshipSponsorsClient';
import { CorporateAccountsClient } from './CorporateAccountsClient';

const tabs = [
  { id: 'scholarship-sponsors', label: 'Scholarship Sponsors' },
...
export default function BillingPolicySetupPage() {
  ...
  const activeTab = (tabs.find((tab) => tab.id === rawTab)?.id) || 'scholarship-sponsors';
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Billing Policy Setup</h1>
              <p className="mt-1 max-w-3xl text-sm text-gray-600 dark:text-gray-400">
                Review sponsor and corporate payer master records that support scholarship coverage, third-party billing, and negotiated LMS customer billing.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
        ...
        <div className="p-[10px]">
          {activeTab === 'scholarship-sponsors' ? (
            <ScholarshipSponsorsClient />
          ) : (
