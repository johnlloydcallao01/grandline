...
import {
  ...
  getDocumentCategoryDetail,
  type DocumentCategoriesRegisterResponse,
  type DocumentCategoryCell,
  type DocumentCategoryDetail,
  type DocumentCategoryMetric,
} from './actions';
...
type DocumentCategoriesPanelProps = {
  title: string;
  description: string;
...
function getMetricTone(trend: DocumentCategoryMetric['trend']) {
...
function renderCell(cell: DocumentCategoryCell, index: number) {
...
export function DocumentCategoriesPanel({
  title,
  description,
  searchPlaceholder,
  tableTitle,
...
}: DocumentCategoriesPanelProps) {
  const [data, setData] = useState<DocumentCategoriesRegisterResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [searchInput, setSearchInput] = useState('');
  const [submittedSearch, setSubmittedSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [filters, setFilters] = useState<FilterState>({ groups: [], statuses: [] });
  const [draftFilters, setDraftFilters] = useState<FilterState>({ groups: [], statuses: [] });
  const [quickFilters, setQuickFilters] = useState<string[]>(defaultQuickFilters);
  const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
  const [detail, setDetail] = useState<DocumentCategoryDetail | null>(null);
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  const [isDetailLoading, setIsDetailLoading] = useState(false);

  useEffect(() => {
    setQuickFilters(defaultQuickFilters);
    setFilters({ groups: [], statuses: [] });
    setDraftFilters({ groups: [], statuses: [] });
    setSearchInput('');
    setSubmittedSearch('');
    setCurrentPage(1);
  }, [defaultQuickFilters]);
  ...
  const fetchRegister = useCallback(
    async ({
      ...
      try {
        ...
        setError(fetchError instanceof Error ? fetchError.message : 'Unable to load document categories.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const filterCount = filters.groups.length + filters.statuses.length;
  ...
  const handleExport = () => {
    ...
    const headers = ['Category', 'Typical Use', 'Linked Records', 'Primary Usage', 'Notes', 'Status'];
    const csvRows = rows.map((row) => [
      row.category,
      row.typicalUse,
      row.linkedRecordCountLabel,
      row.primaryUsageLabel,
      row.notes,
      row.statusLabel,
    ]);
  ...
  const handleViewDetail = async (id: string) => {
    ...
    setDetail(null);
    try {
      setDetail(await getDocumentCategoryDetail(id));
    } catch (detailError) {
      setError(detailError instanceof Error ? detailError.message : 'Unable to load category detail.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <SlideOver
        ...
        title="Document Category Detail"
        description="Review how this backend-defined category is currently used across linked accounting records."
      >
        <div className="space-y-6">
          {isDetailLoading ? (
            <LoadingSkeleton />
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  ...
                  ['Linked Records', detail.linkedRecordCountLabel],
                  ['Primary Usage', detail.primaryUsageLabel],
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Governance Notes</h4>
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Primary Allowed</p>
                    <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.canBePrimary ? 'Yes' : 'No'}</p>
                  </div>
                  <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Receipt Oriented</p>
                    <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.receiptOriented ? 'Yes' : 'No'}</p>
                  </div>
                </div>
                <div className="mt-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Entity Coverage</p>
                  <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.entityCoverageSummary}</p>
                </div>
                <div className="mt-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Recent Linked Records</h4>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Most recent accounting document links currently using this category.
                </p>
                {detail.recentLinks.length > 0 ? (
                  <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 dark:border-[var(--card-border)]">
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        <thead className="bg-gray-50 dark:bg-gray-800/50">
                          <tr>
                            {['Entity Type', 'Entity ID', 'Document Date', 'Linked At', 'Primary'].map((column) => (
                              <th
                                key={column}
                  ...
                  <div className="mt-4 rounded-xl border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 p-4 text-sm text-amber-800 dark:text-amber-400">
                    No linked records currently use this category.
                  </div>
                )}
              </div>
            </>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">No category detail available.</p>
          )}
        </div>
      </SlideOver>
    </div>
