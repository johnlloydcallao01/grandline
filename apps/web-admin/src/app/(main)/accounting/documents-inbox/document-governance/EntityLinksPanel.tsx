...
import {
  getEntityLinkDetail,
  getEntityLinks,
  type EntityLinksCell,
  type EntityLinksDetail,
  type EntityLinkMetric,
  type EntityLinksRegisterResponse,
} from './actions';
...
function getMetricTone(trend: EntityLinkMetric['trend']) {
...
function renderCell(cell: EntityLinksCell, index: number) {
...
export function EntityLinksPanel({
  ...
  const [data, setData] = useState<EntityLinksRegisterResponse | null>(null);
  ...
  const [detail, setDetail] = useState<EntityLinksDetail | null>(null);
  ...
  const fetchRegister = useCallback(
    async ({
      ...
      try {
        ...
        setError(fetchError instanceof Error ? fetchError.message : 'Unable to load entity links.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleExport = () => {
    ...
    const headers = ['Entity Type', 'Linked Records', 'Latest Entity ID', 'Common Category', 'Primary Links', 'Status'];
    const csvRows = rows.map((row) => [
      row.entityTypeLabel,
      row.linkedRecordCountLabel,
      row.latestEntityId,
      row.commonCategoryLabel,
      row.primaryLinkCountLabel,
      row.statusLabel,
    ]);
  ...
  const handleViewDetail = async (id: string) => {
    ...
    setDetail(null);
    try {
      setDetail(await getEntityLinkDetail(id));
    } catch (detailError) {
      setError(detailError instanceof Error ? detailError.message : 'Unable to load entity link detail.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <SlideOver
        ...
        title="Entity Link Detail"
        description="Review how this backend-defined entity target is currently used across linked accounting records."
      >
        <div className="space-y-6">
          {isDetailLoading ? (
            <LoadingSkeleton />
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  ...
                  ['Distinct Record IDs', detail.uniqueEntityIdCountLabel],
                  ['Latest Entity ID', detail.latestEntityId],
                  ['Common Category', detail.commonCategoryLabel],
                  ['Latest Linked At', detail.latestLinkedAtLabel],
                  ['Primary Links', detail.primaryLinkCountLabel],
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                ...
                <div className="mt-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Active Entity IDs</p>
                  <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.activeEntityIdSummary}</p>
                </div>
                <div className="mt-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                ...
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Most recent accounting document links currently targeting this entity type.
                </p>
                {detail.recentLinks.length > 0 ? (
                  ...
                  <div className="mt-4 rounded-xl border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 p-4 text-sm text-amber-800 dark:text-amber-400">
                    No linked records currently target this entity type.
                  </div>
                )}
              </div>
            </>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">No entity link detail available.</p>
          )}
        </div>
      </SlideOver>
    </div>
