...
import {
  getDocumentLibraryLinkedDocumentDetail,
  getDocumentLibraryLinkedDocuments,
  type LinkedDocumentsCell,
  type LinkedDocumentsDetail,
  type LinkedDocumentsMetric,
  type LinkedDocumentsRegisterResponse,
} from './actions';
...
function getMetricTone(trend: LinkedDocumentsMetric['trend']) {
...
function renderCell(cell: LinkedDocumentsCell, index: number) {
...
export function LinkedDocumentsPanel({
  ...
  const [data, setData] = useState<LinkedDocumentsRegisterResponse | null>(null);
  ...
  const [detail, setDetail] = useState<LinkedDocumentsDetail | null>(null);
  ...
  const fetchRegister = useCallback(
    async ({
      ...
      setError(null);
      try {
        const response = await getDocumentLibraryLinkedDocuments({
          search,
          page,
        ...
        setError(fetchError instanceof Error ? fetchError.message : 'Unable to load linked documents.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleExport = () => {
    ...
    const headers = ['File Name', 'Entity Type', 'Entity ID', 'Category', 'Is Primary', 'Status'];
  ...
  const handleViewDetail = async (id: string) => {
    ...
    setDetail(null);
    try {
      setDetail(await getDocumentLibraryLinkedDocumentDetail(id));
    } catch (detailError) {
      setError(detailError instanceof Error ? detailError.message : 'Unable to load linked document detail.');
    } finally {
  ...
  return (
    <div className="space-y-6">
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="flex justify-end gap-2">
                                <button
                                  ...
                                  title="View linked document detail"
                                >
      ...
      <SlideOver
        ...
        title="Linked Document Detail"
        description="Review the linked accounting document, file metadata, and source context."
      >
        <div className="space-y-6">
          {isDetailLoading ? (
            <LoadingSkeleton />
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                      Open the linked file directly from Cloudinary or the stored media URL.
                    </p>
                  </div>
                  {detail.fileUrl ? (
                    <a
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">No linked document detail available.</p>
          )}
        </div>
      </SlideOver>
    </div>
