...
import {
  getDocumentLibraryAllFileDetail,
  getDocumentLibraryAllFiles,
  type AllFilesCell,
  type AllFilesDetail,
  type AllFilesMetric,
  type AllFilesRegisterResponse,
} from './actions';
...
function getMetricTone(trend: AllFilesMetric['trend']) {
...
function renderCell(cell: AllFilesCell, index: number) {
...
export function AllFilesPanel({
  ...
  const [data, setData] = useState<AllFilesRegisterResponse | null>(null);
  ...
  const [detail, setDetail] = useState<AllFilesDetail | null>(null);
  ...
  const fetchRegister = useCallback(
    async ({
      ...
      setError(null);
      try {
        const response = await getDocumentLibraryAllFiles({
          search,
          page,
        ...
        setError(fetchError instanceof Error ? fetchError.message : 'Unable to load document library.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleToggleQuickFilter = (value: string) => {
    if (value === 'scope:all') {
      setQuickFilters(['scope:all']);
      setCurrentPage(1);
      return;
    }

    setQuickFilters((previous) => {
      const nextValues = toggleFilterValue(previous.filter((entry) => entry !== 'scope:all'), value);
      return nextValues.length ? nextValues : ['scope:all'];
    });
    setCurrentPage(1);
  };
  ...
  const handleViewDetail = async (id: string) => {
    ...
    setDetail(null);
    try {
      setDetail(await getDocumentLibraryAllFileDetail(id));
    } catch (detailError) {
  ...
  return (
    <div className="space-y-6">
          ...
          {isFilterPanelOpen ? (
            <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-3">
                <div>
                  <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">File Type</h5>
      ...
      <SlideOver
        ...
        title="File Detail"
        description="Review file metadata, accounting linkage coverage, and open the stored file when needed."
      >
        <div className="space-y-6">
          {isDetailLoading ? (
            <LoadingSkeleton />
            ...
            <>
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Linkage Summary</h4>
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Document Categories</p>
                    <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.usageSummary.categorySummary}</p>
                  </div>
                  <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Entity Types</p>
                    <p className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{detail.usageSummary.entityTypeSummary}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4 shadow-sm">
                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Linked Accounting Records</h4>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Document-link records currently attached to this file.
                </p>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">No file detail available.</p>
          )}
        </div>
      </SlideOver>
    </div>
