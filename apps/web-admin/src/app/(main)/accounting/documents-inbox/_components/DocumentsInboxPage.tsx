...
type DocumentsBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type DocumentsTableCell =
  | string
  | {
      text: string;
      tone?: DocumentsBadgeTone;
      emphasis?: boolean;
...
type DocumentsTableRow = {
  id: string;
  cells: DocumentsTableCell[];
};

export type DocumentsInboxTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: DocumentsMetric[];
...
type DocumentsInboxPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: DocumentsInboxTab[];
  headerActions?: DocumentsAction[];
};

function getActionIcon(icon?: DocumentsAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: DocumentsAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: DocumentsBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: DocumentsTableCell, index: number) {
...
function MetricCard({ metric }: { metric: DocumentsMetric }) {
