...
type ApprovalBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';
...
type ApprovalsPageProps = {
  ...
  description: string;
  tabs: ApprovalTab[];
  headerActions?: ApprovalAction[];
};

function getActionIcon(icon?: ApprovalAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: ApprovalAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: ApprovalBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: ApprovalTableCell, index: number) {
...
function MetricCard({ metric }: { metric: ApprovalMetric }) {
