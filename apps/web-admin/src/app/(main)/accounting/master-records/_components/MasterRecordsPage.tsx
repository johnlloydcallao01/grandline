...
type MasterBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type MasterTableCell =
  | string
  | {
      text: string;
      tone?: MasterBadgeTone;
      emphasis?: boolean;
...
type MasterTableRow = {
  id: string;
  cells: MasterTableCell[];
};

export type MasterRecordsTab = {
  id: string;
  label: string;
  description: string;
...
type MasterRecordsPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: MasterRecordsTab[];
  headerActions?: MasterAction[];
};

function getActionIcon(icon?: MasterAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: MasterAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: MasterBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: MasterTableCell, index: number) {
...
function MetricCard({ metric }: { metric: MasterMetric }) {
