...
function statusBadge(status: string) {
  const colors: Record<string, string> = {
    ...
    closed: 'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-900/50 dark:text-slate-400 dark:border-slate-700',
    soft_locked: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800',
    draft: 'bg-gray-50 text-gray-500 border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700',
  }
...
function closeModeBadge(mode: string) {
  if (mode === 'hard_lock') {
    return (
      <span className="inline-flex items-center gap-1 rounded-md border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 px-2 py-0.5 text-xs font-medium text-red-700 dark:text-red-400">
        <Lock className="h-3 w-3" /> Hard Lock
      </span>
    )
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-md border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800 px-2 py-0.5 text-xs font-medium text-gray-600 dark:text-gray-400">
  ...
  function renderConfirmModal() {
    ...
    return createPortal(
      <div className="fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ease-in-out bg-black/50">
        <div className="w-full max-w-md rounded-xl bg-white dark:bg-[var(--card-background)] shadow-2xl">
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-[var(--card-border)] px-6 py-4">
            ...
            <button type="button" onClick={() => { setConfirmAction(null); setActionError(null); setLockedFromDate(''); setClearLockDate(false) }} className="rounded-lg p-1 text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-300">
              <X className="h-5 w-5" />
            </button>
          </div>
          <div className="px-6 py-5 space-y-4">
            ...
            {isClose ? (
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Lock From Date (optional)</label>
                <input type="date" value={lockedFromDate} onChange={(e) => setLockedFromDate(e.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-3 py-2 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-800" />
                <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Leave blank to use the period/fiscal year end date.</p>
              </div>
            ) : null}
            {!isClose ? (
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 dark:border-[var(--card-border)] px-3 py-2.5">
                <input type="checkbox" checked={clearLockDate} onChange={(e) => setClearLockDate(e.target.checked)} className="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-blue-600 outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-800" />
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Clear lock date on reopen</span>
              </label>
            ) : null}
          </div>
          <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-[var(--card-border)] px-6 py-4">
            <button type="button" onClick={() => { setConfirmAction(null); setActionError(null); setLockedFromDate(''); setClearLockDate(false) }} disabled={isSubmitting} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50">
              Cancel
            </button>
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">Close & Approval Controls</h1>
          <p className="mt-1 text-base text-gray-600 dark:text-gray-400">
            Command center for period-end close, approval workflow coverage, and posting safeguards.
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-3">
          <button type="button" onClick={() => { void loadState(); void loadCoverage() }} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800">
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input type="text" value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)}
              ...
              className="min-w-0 flex-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] px-3 py-2 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-800 md:max-w-xs"
            />
          ...
          <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-200 dark:border-[var(--card-border)]">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Fiscal Years & Periods</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">Click a fiscal year to expand its periods. Use action buttons to close or reopen.</p>
            </div>
            {isLoadingState ? (
              <LoadingSkeleton />
              ...
              <div>
                <div className="hidden md:grid md:grid-cols-[2fr_1fr_1fr_1fr_1.5fr_auto] gap-4 px-5 py-2.5 bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-[var(--card-border)] text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                ...
                {filteredFiscalYears.map((fy) => {
                  ...
                  return (
                    <div key={String(fy.id)}>
                      <div
                        className={`grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_1.5fr_auto] gap-2 md:gap-4 px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 items-center cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${isExpanded ? 'bg-gray-50/50 dark:bg-gray-800/30' : ''}`}
                        onClick={() => toggleFiscalYear(fy.id)}
                      >
                        <div className="flex items-center gap-2 font-medium text-gray-900 dark:text-gray-100">
                          {isExpanded ? <ChevronDown className="h-4 w-4 shrink-0 text-gray-400 dark:text-gray-500" /> : <ChevronRight className="h-4 w-4 shrink-0 text-gray-400 dark:text-gray-500" />}
                          {fy.code}
                          <span className="text-xs text-gray-500 dark:text-gray-400 font-normal">— {fy.name}</span>
                        </div>
                        <span className="text-sm text-gray-600 dark:text-gray-400">{fy.startDate?.slice(0, 10)} to {fy.endDate?.slice(0, 10)}</span>
                        <div className="flex items-center gap-2">{statusBadge(fy.status)}{closeModeBadge(fy.closeMode)}</div>
                        <span className="text-sm text-gray-500 dark:text-gray-400">{fy.lockedFromDate ? fy.lockedFromDate.slice(0, 10) : '—'}</span>
                        <span className="text-sm text-gray-500 dark:text-gray-400">{fy.closedAt ? fy.closedAt.slice(0, 10) : '—'}</span>
                        <div className="flex gap-2 w-40" onClick={(e) => e.stopPropagation()}>
                          {fy.status !== 'closed' ? (
                            <button type="button" onClick={() => { setConfirmAction({ type: 'closeFiscalYear', fiscalYear: fy }); setLockedFromDate('') }}
                              className="rounded-md border border-red-200 dark:border-red-800 bg-white dark:bg-[var(--card-background)] px-2.5 py-1 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30">Close</button>
                          ) : (
                            <button type="button" onClick={() => { setConfirmAction({ type: 'reopenFiscalYear', fiscalYear: fy }); setClearLockDate(false) }}
                              className="rounded-md border border-blue-200 dark:border-blue-800 bg-white dark:bg-[var(--card-background)] px-2.5 py-1 text-xs font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30">Reopen</button>
                          )}
                        </div>
                      </div>
                      {isExpanded && fy.periods.length > 0 && (
                        <div className="bg-gray-50/50 dark:bg-gray-800/30">
                          <div className="hidden md:grid md:grid-cols-[2fr_1fr_1fr_1fr_1.5fr_auto] gap-4 px-5 py-1.5 text-xs text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-gray-800">
                            <span>Period</span>
                            <span>Range</span>
                            <span>Status</span>
                            <span>Locked From</span>
                          ...
                          {fy.periods.map((p) => (
                            <div key={String(p.id)} className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_1.5fr_auto] gap-2 md:gap-4 px-5 py-3 border-b border-gray-100 dark:border-gray-800 items-center hover:bg-white dark:hover:bg-[var(--card-background)] transition-colors">
                              <span className="text-sm font-medium text-gray-800 dark:text-gray-200 pl-4 md:pl-8">{p.label}</span>
                              <span className="text-sm text-gray-600 dark:text-gray-400">{p.startDate?.slice(0, 10)} to {p.endDate?.slice(0, 10)}</span>
                              <div>{statusBadge(p.status)}</div>
                              <span className="text-sm text-gray-500 dark:text-gray-400">{p.lockedFromDate ? p.lockedFromDate.slice(0, 10) : '—'}</span>
                              <span className="text-sm text-gray-500 dark:text-gray-400">{p.closedAt ? p.closedAt.slice(0, 10) : '—'}</span>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-[var(--card-border)] bg-gray-50/50 dark:bg-gray-800/50">
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Entity Type</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Active Workflows</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Workflow Codes</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Steps</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {filteredEntityCoverage.length === 0 ? (
                    ...
                    filteredEntityCoverage.map((ec) => (
                      <tr key={ec.entityType} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                        <td className="px-5 py-3.5 font-medium text-gray-900 dark:text-gray-100">{ec.entityLabel}</td>
                        <td className="px-5 py-3.5">
                          <span className={`inline-flex items-center gap-1.5 font-medium ${ec.hasActiveWorkflow ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                        ...
                        <td className="px-5 py-3.5">
                          {ec.workflows.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {ec.workflows.map((wf) => (
                                <span key={wf.workflowCode} className="inline-flex rounded-md bg-blue-50 dark:bg-blue-950/30 px-2 py-0.5 text-xs font-medium text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
                                  {wf.workflowCode}
                                </span>
                              ))}
                            </div>
      ...
      {activeTab === 'safeguards' && (
        <>
          ...
          <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-200 dark:border-[var(--card-border)]">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">All Safeguards Registry</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-[var(--card-border)] bg-gray-50/50 dark:bg-gray-800/50">
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Control Area</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Protected Action</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Condition</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Behavior</th>
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Source</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {(coverageData?.safeguards ?? []).length === 0 ? (
                    ...
                    (coverageData?.safeguards ?? []).map((g, i) => (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                        <td className="px-5 py-3.5 font-medium text-gray-900 dark:text-gray-100">{g.area}</td>
                        <td className="px-5 py-3.5 text-gray-700 dark:text-gray-300">{g.protectedAction}</td>
                        <td className="px-5 py-3.5 text-gray-600 dark:text-gray-400 max-w-xs">{g.condition}</td>
                        <td className="px-5 py-3.5 text-gray-600 dark:text-gray-400 max-w-xs">{g.behavior}</td>
                        <td className="px-5 py-3.5">
                          <span className="inline-flex rounded-md bg-purple-50 dark:bg-purple-950/30 px-2 py-0.5 text-xs font-medium text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800">
                            {g.source}
                          </span>
                        </td>
                      </tr>
