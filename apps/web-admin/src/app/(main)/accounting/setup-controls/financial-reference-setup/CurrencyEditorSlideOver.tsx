...
import { fetchCurrency, updateCurrency, deleteCurrency, type CurrencyRecord, type UpdateCurrencyData } from './actions'

type CurrencyEditorSlideOverProps = {
...
export function CurrencyEditorSlideOver({
  currencyId,
  isOpen,
  onClose,
  onUpdated,
  onDeleted,
}: CurrencyEditorSlideOverProps) {
  const [mounted, setMounted] = useState(false)
  const [animate, setAnimate] = useState(false)
  const [record, setRecord] = useState<CurrencyRecord | null>(null)
  ...
  const loadRecord = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const res = await fetchCurrency(currencyId)
      setRecord(res)
      setForm({
        code: res.code,
        name: res.name,
        symbol: res.symbol ?? '',
        isBaseCurrency: res.isBaseCurrency ?? false,
      ...
      setLoadError(err instanceof Error ? err.message : 'Failed to load currency.')
    } finally {
      setIsLoading(false)
    }
  }, [currencyId])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const editableFields = ['code', 'name', 'symbol', 'isBaseCurrency', 'isActive', 'notes'] as const
      for (const field of editableFields) {
        if (form[field] !== undefined) {
          (updateData as Record<string, unknown>)[field] = form[field]
        }
      }
      const res = await updateCurrency(currencyId, updateData)
      setRecord(res)
      setForm({
        code: res.code,
        name: res.name,
      ...
      addToast({ title: 'Currency updated', message: `${res.code} has been updated.`, type: 'success' })
      onUpdated()
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast({ title: 'Currency deleted', message: 'The currency has been deleted.', type: 'info' })
      onDeleted()
      onClose()
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Failed to delete currency.')
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-[var(--card-border)] px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            {isLoading ? 'Loading...' : record ? `${record.code} — ${record.name}` : 'Currency Details'}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <section>
                <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Currency Information</h3>
                ...
                <div className="mt-4">
                  <FieldLabel label="Symbol" htmlFor="slide-symbol" />
                  <input
                    id="slide-symbol"
                    type="text"
                ...
                <div className="mt-4">
                  <FieldLabel label="Notes" htmlFor="slide-notes" />
                  <textarea
                    id="slide-notes"
              ...
              <section>
                <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Flags</h3>
                <div className="grid grid-cols-2 gap-4">
                  <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 dark:border-[var(--card-border)] px-3 py-2.5">
                    <input
                      type="checkbox"
                      checked={(form.isBaseCurrency as boolean) ?? false}
                      onChange={(e) => updateField('isBaseCurrency', e.target.checked)}
                ...
                <div className="rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-red-800 dark:text-red-300">Delete this currency?</p>
