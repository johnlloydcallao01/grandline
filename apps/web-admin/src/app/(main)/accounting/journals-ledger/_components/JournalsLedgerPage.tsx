...
type JournalBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type JournalTableCell =
  | string
  | {
      text: string;
      tone?: JournalBadgeTone;
      emphasis?: boolean;
...
type JournalTableRow = {
  id: string;
  cells: JournalTableCell[];
};

export type JournalsLedgerTab = {
  id: string;
  label: string;
  description: string;
...
type JournalsLedgerPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: JournalsLedgerTab[];
  headerActions?: JournalAction[];
};

function getActionIcon(icon?: JournalAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: JournalAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: JournalBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: JournalTableCell, index: number) {
...
function MetricCard({ metric }: { metric: JournalMetric }) {
...
export function JournalsLedgerPage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: JournalsLedgerPageProps) {
