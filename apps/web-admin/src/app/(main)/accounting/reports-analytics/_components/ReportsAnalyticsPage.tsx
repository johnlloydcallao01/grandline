...
type ReportsBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type ReportsTableCell =
  | string
  | {
      text: string;
      tone?: ReportsBadgeTone;
      emphasis?: boolean;
...
type ReportsAnalyticsPageProps = {
  ...
  title: string;
  description: string;
  tabs: ReportsAnalyticsTab[];
  headerActions?: ReportsAction[];
};

function getActionIcon(icon?: ReportsAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: ReportsAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: ReportsBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: ReportsTableCell, index: number) {
