...
import { getExpenseReports, type ExpenseReportsResponse, type ExpenseReportRow, type ErMetric } from './actions';
...
function getMetricTone(t: ErMetric['trend']) {
...
export function ExpenseReportsClient() {
  const [data, setData] = useState<ExpenseReportsResponse | null>(null);
  ...
  const [viewRow, setViewRow] = useState<ExpenseReportRow | null>(null);
  ...
  const fetchData = useCallback(async ({ search, page, nextFilters, nextQuickFilters }: { search: string; page: number; nextFilters: string[]; nextQuickFilters: string[] }) => {
    ...
    try { const r = await getExpenseReports({ search, page, statuses: nextFilters, quickFilters: nextQuickFilters }); setData(r); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to load expense reports.'); } finally { setIsLoading(false); }
  }, []);
  ...
  const handleExport = () => {
    ...
    const headers = ['Expense No.', 'Date', 'Vendor', 'Currency', 'Total', 'Status'];
    const csvRows = rows.map((r) => [r.documentNumber, r.documentDateLabel, r.partyName, r.currency, formatCurrency(r.total), r.statusLabel]);
    const csvContent = [headers.map((h) => escapeCsvValue(h)), ...csvRows.map((r) => r.map((c) => escapeCsvValue(c)))].map((r) => r.join(',')).join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'expense-reports.csv'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
  };

  const handleViewRow = (row: ExpenseReportRow) => { setViewRow(row); setIsViewOpen(true); };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Expense Reports</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">Review posted expense-register output using expense number, expense date, vendor, and total amount fields.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{data?.totals.filteredRows ?? 0} matching rows</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={handleRefresh} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('secondary')}`}><RefreshCw className="h-4 w-4" /> Refresh Expenses</button>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder="Search expense no., expense date, vendor, status, or amount" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 dark:focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          {isFilterPanelOpen && (
            <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => { setDraftFilters([]); setFilters([]); setCurrentPage(1); setIsFilterPanelOpen(false); }} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">Clear all</button>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(data?.section?.filters?.statuses || []).map((opt) => {
                    const isSelected = draftFilters.includes(opt.value);
                    return <button key={opt.value} type="button" onClick={() => setDraftFilters((p) => toggleFilterValue(p, opt.value))} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${isSelected ? 'bg-blue-600 dark:bg-blue-700 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{opt.label}</button>;
                  })}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Expense Register</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Expense report rows aligned to the exposed expense-register endpoint in apps/cms.</p>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Expense No.</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Date</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Vendor</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Currency</th>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[var(--card-background)]">
                      ...
                      )) : (<tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No expense report rows found.</td></tr>)}
                    </tbody>
                  </table>
                </div>
              </div>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Expense Detail" description="View expense report transaction details.">
        <div className="space-y-6">
          {viewRow ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-5 shadow-sm space-y-4">
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Expense No.</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.documentNumber}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Date</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.documentDateLabel}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Vendor</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.partyName}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Currency</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.currency}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Total</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatCurrency(viewRow.total)}</span></div>
