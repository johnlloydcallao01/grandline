...
import { getTrialBalanceRegister, type TrialBalanceRegisterResponse, type TrialBalanceRegisterRow, type TbMetric } from './actions';
...
function getMetricTone(t: TbMetric['trend']) {
...
export function TrialBalanceClient() {
  const [data, setData] = useState<TrialBalanceRegisterResponse | null>(null);
  ...
  const [viewRow, setViewRow] = useState<TrialBalanceRegisterRow | null>(null);
  ...
  const fetchData = useCallback(async ({
    ...
    try {
      const r = await getTrialBalanceRegister({ search, page, statuses: nextFilters.statuses, quickFilters: nextQuickFilters });
      setData(r);
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'trial-balance.csv'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
  };

  const handleViewRow = (row: TrialBalanceRegisterRow) => { setViewRow(row); setIsViewOpen(true); };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Trial Balance Report</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Trial-balance rows aligned to the real trial-balance service and query support in apps/cms.</p>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Account Code</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Account Name</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Type</th>
                        <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Total Debit</th>
                        <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Total Credit</th>
                        <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Closing Balance</th>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[var(--card-background)]">
                      ...
                      )) : (<tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No trial balance rows found.</td></tr>)}
                    </tbody>
                  </table>
                </div>
              </div>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Account Detail" description="View trial balance account details.">
        <div className="space-y-6">
          {viewRow ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-5 shadow-sm space-y-4">
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Account Code</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.accountCode ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Account Name</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.accountName ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Account Type</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{viewRow.accountType ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Total Debit</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatCurrency(viewRow.totalDebit)}</span></div>
                <div className="flex justify-between border-b border-gray-100 dark:border-gray-800 pb-3"><span className="text-sm text-gray-500 dark:text-gray-400">Total Credit</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatCurrency(viewRow.totalCredit)}</span></div>
                <div className="flex justify-between"><span className="text-sm text-gray-500 dark:text-gray-400">Closing Balance</span><span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatCurrency(viewRow.closingBalance)}</span></div>
              </div>
            </div>
