    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-gray-600">Browse posted and reversed journal-line activity by account with posting dates, descriptions, and running balances.</p>
          <p className="text-sm text-gray-500">{data?.totals.filteredEntries ?? 0} matching rows</p>
        </div>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder="Search account code, account name, or entry no." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Posting Date</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Entry No.</th>
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      ...
                      )) : (<tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500">No ledger entries found.</td></tr>)}
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          {viewRow ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
                <div className="flex justify-between border-b border-gray-100 pb-3"><span className="text-sm text-gray-500">Posting Date</span><span className="text-sm font-medium text-gray-900">{viewRow.postingDate ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 pb-3"><span className="text-sm text-gray-500">Entry No.</span><span className="text-sm font-medium text-gray-900">{viewRow.entryNumber ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 pb-3"><span className="text-sm text-gray-500">Account</span><span className="text-sm font-medium text-gray-900">{viewRow.account ?? '-'}</span></div>
                <div className="flex justify-between border-b border-gray-100 pb-3"><span className="text-sm text-gray-500">Debit</span><span className="text-sm font-medium text-gray-900">{formatCurrency(viewRow.debit)}</span></div>
                <div className="flex justify-between border-b border-gray-100 pb-3"><span className="text-sm text-gray-500">Credit</span><span className="text-sm font-medium text-gray-900">{formatCurrency(viewRow.credit)}</span></div>
                <div className="flex justify-between"><span className="text-sm text-gray-500">Running Balance</span><span className="text-sm font-medium text-gray-900">{formatCurrency(viewRow.runningBalance)}</span></div>
              </div>
            </div>
