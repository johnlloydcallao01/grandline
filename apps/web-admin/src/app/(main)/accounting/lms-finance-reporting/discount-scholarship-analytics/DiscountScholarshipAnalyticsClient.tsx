    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900">Coupon Revenue Impact</h2>
          <p className="text-sm text-gray-600">{data?.tab === 'coupon-revenue-impact' ? 'Coupon-level revenue impact aligned to coupon redemption snapshots.' : 'Review coupon-level revenue impact.'}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-5 py-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-900">Discount Distribution</h3>
          </div>
          <div className="mt-3">
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder="Search coupon code" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        {['Coupon Code', 'Enrollments', 'Gross Revenue', 'Discount Amount', 'Net Revenue', 'Impact Ratio'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 ${column === 'Enrollments' || column === 'Gross Revenue' || column === 'Discount Amount' || column === 'Net Revenue' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {couponRows.length > 0 ? couponRows.map((row) => (
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500">No coupon revenue impact rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          <div className="space-y-4">
            <div className="grid gap-4">
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Coupon Code</p>
                <p className="mt-1 text-lg font-semibold text-gray-900">{viewDetail.couponCode}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Enrollments</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.enrollmentCount}</p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Impact Ratio</p>
                  <p className="mt-2">
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${
                      viewDetail.impactRatioTone === 'amber' ? 'bg-amber-50 text-amber-700 ring-amber-200' :
                      viewDetail.impactRatioTone === 'blue' ? 'bg-blue-50 text-blue-700 ring-blue-200' :
                      viewDetail.impactRatioTone === 'green' ? 'bg-green-50 text-green-700 ring-green-200' :
                      viewDetail.impactRatioTone === 'red' ? 'bg-red-50 text-red-700 ring-red-200' :
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Gross Revenue</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.grossRevenueLabel}</p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Discount Amount</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.discountAmountLabel}</p>
                </div>
              </div>
              <div className="rounded-xl border border-gray-200 bg-white p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Net Revenue</p>
                <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.netRevenueLabel}</p>
              </div>
            </div>
...
function ScholarshipUtilizationTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900">Scholarship Utilization</h2>
          <p className="text-sm text-gray-600">{data?.tab === 'scholarship-utilization' ? 'Sponsor-level scholarship utilization aligned to active scholarship awards.' : 'Review sponsor-level scholarship utilization.'}</p>
          <p className="text-sm text-gray-500">{data?.totals.filteredRows ?? 0} matching rows</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={handleRefresh} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('secondary')}`}>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-5 py-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-900">Sponsor Award Distribution</h3>
          </div>
          <div className="mt-3">
        ...
        <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Search sponsor code or name" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
              <button type="submit" className="inline-flex items-center gap-2 rounded-lg border border-blue-600 bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:border-blue-700 hover:bg-blue-700">
                <Search className="h-4 w-4" /> Search
              </button>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        {['Sponsor Code', 'Sponsor Name', 'Award Count', 'Awarded Amount', 'Billed Sponsor Amount', 'Trainee Share'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 ${column === 'Award Count' || column === 'Awarded Amount' || column === 'Billed Sponsor Amount' || column === 'Trainee Share' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                        <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {scholarshipRows.length > 0 ? scholarshipRows.map((row) => (
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500">No scholarship utilization rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Scholarship Utilization Detail" description="Review sponsor award amounts, billed sponsor amount, and trainee share." width="max-w-lg">
        {viewDetail ? (
          <div className="space-y-4">
            <div className="grid gap-4">
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Sponsor</p>
                <p className="mt-1 text-lg font-semibold text-gray-900">{viewDetail.sponsorName}</p>
                <p className="text-sm text-gray-500">{viewDetail.sponsorCode}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Award Count</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.awardCount}</p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Awarded Amount</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.awardedAmountLabel}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Billed Sponsor Amount</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.billedSponsorAmountLabel}</p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-white p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Trainee Share</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900">{viewDetail.traineeShareAmountLabel}</p>
                </div>
              </div>
            </div>
            <div className="flex justify-end border-t border-gray-200 pt-4">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Discount & Scholarship Analytics</h1>
              <p className="mt-1 max-w-3xl text-base text-gray-600">Review coupon revenue impact and scholarship utilization using the dedicated LMS report queries already exposed in the backend.</p>
            </div>
          </div>
        </div>
      </div>
