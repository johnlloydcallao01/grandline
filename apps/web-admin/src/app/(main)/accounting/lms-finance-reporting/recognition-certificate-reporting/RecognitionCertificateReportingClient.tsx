    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{data?.section.label || 'Completion To Revenue Report'}</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">{data?.section.description || 'Review completed LMS enrollments against billed revenue, recognized revenue, and remaining deferred revenue.'}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{data?.section.table.description}</p>
        </div>
          ...
          <div className="flex flex-1 flex-col gap-3 lg:flex-row lg:items-center">
            <form onSubmit={handleSearch} className="flex min-w-0 max-w-xl flex-1 gap-3">
              <div className="relative min-w-0 flex-1">
                ...
                <input type="text" placeholder={data?.section.searchPlaceholder || STATIC_TABS[0].searchPlaceholder} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[var(--card-background)] py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </div>
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.section.table.title || 'Completion To Revenue Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.section.table.description || 'Recognition view for completed LMS enrollments.'}</p>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Enrollment', 'Course', 'Completed At', 'Final Charge', 'Recognized Revenue', 'Remaining Deferred'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Final Charge' || column === 'Recognized Revenue' || column === 'Remaining Deferred' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[var(--card-background)]">
                      {completionRows.length > 0 ? completionRows.map((row) => (
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No completion-to-revenue rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
          ...
          <div className="space-y-4">
            <div className="grid gap-4">
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Enrollment</p>
                <p className="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{viewDetail.enrollmentRef}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Completed At</p>
                  <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.completedAtLabel}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Final Charge</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{viewDetail.finalChargeLabel}</p>
                </div>
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Recognized Revenue</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{viewDetail.recognizedRevenueLabel}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Remaining Deferred</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{viewDetail.deferredRevenueLabel}</p>
                </div>
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Billing Status</p>
                  <p className="mt-2">
...
function CertificateRevenueTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{data?.section.label || 'Certificate Revenue Report'}</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">{data?.section.description || 'Review certificate-issued revenue rows using the dedicated certificate revenue query.'}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{data?.section.table.description}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={handleRefresh} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${getActionClasses('secondary')}`}>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] shadow-sm">
        ...
        <div className="space-y-4 p-[10px] sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{data?.section.table.title || 'Certificate Revenue Register'}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{data?.section.table.description || 'Certificate revenue view including issue date and billed amount.'}</p>
            </div>
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
              <span>{data?.totals.filteredRows ?? 0} matching rows</span>
            </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        {['Certificate ID', 'Enrollment', 'Course', 'Issue Date', 'Billed Amount', 'Billing State'].map((column) => (
                          <th key={column} className={`px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${column === 'Billed Amount' ? 'text-right' : 'text-left'}`}>{column}</th>
                        ))}
                        <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[var(--card-background)]">
                      {certificateRows.length > 0 ? certificateRows.map((row) => (
                        ...
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No certificate revenue rows found.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <SlideOver isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Certificate Revenue Detail" description="Review certificate issuance values and billing state." width="max-w-lg">
        {viewDetail ? (
          <div className="space-y-4">
            <div className="grid gap-4">
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Certificate</p>
                <p className="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{viewDetail.certificateCode}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Enrollment</p>
                  <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.enrollmentRef}</p>
                </div>
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Course</p>
                  <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.courseTitle}</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Issue Date</p>
                  <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{viewDetail.issueDateLabel}</p>
                </div>
                <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Billed Amount</p>
                  <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{viewDetail.billedAmountLabel}</p>
                </div>
              </div>
              <div className="rounded-xl border border-gray-200 dark:border-[var(--card-border)] bg-white dark:bg-[var(--card-background)] p-4">
                ...
                <p className="mt-2">
                  <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${
                    viewDetail.billingStateTone === 'green' ? 'bg-green-50 text-green-700 ring-green-200 dark:bg-green-950/30 dark:text-green-400 dark:ring-green-800' :
                    viewDetail.billingStateTone === 'amber' ? 'bg-amber-50 text-amber-700 ring-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:ring-amber-800' :
                    'bg-gray-100 text-gray-700 ring-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-700'
    ...
    <div className="space-y-6 p-[10px]">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">Recognition & Certificate Reporting</h1>
          <p className="mt-1 text-base text-gray-600 dark:text-gray-400">Review recognition timing and certificate-fee reporting using the dedicated LMS completion-to-revenue and certificate revenue report queries.</p>
        </div>
      </div>
