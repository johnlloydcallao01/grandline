...
type LmsFinanceAction = {
  label: string;
  icon?: 'plus' | 'upload' | 'download' | 'refresh';
  variant?: 'primary' | 'secondary' | 'ghost';
};

type LmsFinanceMetric = {
  label: string;
  value: string;
  change: string;
...
type LmsFinanceBadgeTone = 'gray' | 'blue' | 'green' | 'amber' | 'red';

type LmsFinanceTableCell =
  | string
  | {
      text: string;
      tone?: LmsFinanceBadgeTone;
      emphasis?: boolean;
      align?: 'left' | 'right' | 'center';
    };

type LmsFinanceTableRow = {
  id: string;
  cells: LmsFinanceTableCell[];
};

export type LmsFinanceReportingTab = {
  id: string;
  label: string;
  description: string;
  searchPlaceholder: string;
  filters: string[];
  metrics: LmsFinanceMetric[];
  actions: LmsFinanceAction[];
  tableTitle: string;
  tableDescription: string;
  columns: string[];
  rows: LmsFinanceTableRow[];
};

type LmsFinanceReportingPageProps = {
  eyebrow: string;
  title: string;
  description: string;
  tabs: LmsFinanceReportingTab[];
  headerActions?: LmsFinanceAction[];
};

function getActionIcon(icon?: LmsFinanceAction['icon']) {
  switch (icon) {
    case 'plus':
      return Plus;
    case 'upload':
...
function getActionClasses(variant: LmsFinanceAction['variant'] = 'secondary') {
  if (variant === 'primary') {
...
function getBadgeClasses(tone: LmsFinanceBadgeTone = 'gray') {
  switch (tone) {
    case 'blue':
...
function renderCell(cell: LmsFinanceTableCell, index: number) {
...
function MetricCard({ metric }: { metric: LmsFinanceMetric }) {
...
export function LmsFinanceReportingPage({
  eyebrow,
  title,
  description,
  tabs,
  headerActions,
}: LmsFinanceReportingPageProps) {
