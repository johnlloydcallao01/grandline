...
import {
    Search, Eye, Edit, Loader2, X, GraduationCap, BookOpen, Users,
    CheckCircle, Plus
...
function EvalBadge({ eval: ev }: { eval: string | null | undefined }) {
    if (!ev) return <span className="text-xs text-gray-400 dark:text-gray-500">—</span>;
...
export default function CourseGradebookPage() {
    ...
    const loadCourse = useCallback(async () => {
        try {
            ...
            const found = data.docs.find(c => c.id === Number(courseId));
    ...
    const loadEnrollments = useCallback(async () => {
        try {
            setIsLoading(true);
            setError(null);
            const data = await getEnrollmentsList({
    ...
    }, [debouncedSearch, currentPage, courseId]);
    ...
    const openDetail = async (e: EnrollmentDoc) => {
        ...
        try {
            const full = await getEnrollmentById(e.id);
            setDetailEnrollment(full);
    ...
    const avgGrade = enrollments.length > 0
        ? Math.round(enrollments.reduce((sum, e) => sum + (e.currentGrade ?? 0), 0) / enrollments.length)
        : 0;
        ...
        return (
            <div className="p-6 flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    ...
                    <p className="text-gray-900 dark:text-gray-100 font-medium mb-2">Failed to load course gradebook</p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">{error}</p>
                    <button onClick={loadEnrollments} className="px-4 py-2 bg-blue-600 dark:bg-blue-500 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 text-sm font-medium">Retry</button>
                </div>
            </div>
        );
    }
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{course?.title || `Course #${courseId}`}</h1>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gradebook &mdash; student grades and evaluations</p>
                    </div>
                </div>
                        <Link href={`/gradebook/create?courseId=${courseId}`}
                ...
                {isLoading ? (
                    ...
                    <>
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/30"><Users className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
                                <div>
                        ...
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-lg bg-amber-50 dark:bg-amber-900/30"><BookOpen className="h-5 w-5 text-amber-600 dark:text-amber-400" /></div>
                                <div>
                                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{avgGrade}%</p>
                        ...
                        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] p-4 shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{passedCount}</p>
            ...
            {isLoading ? (
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-[var(--card-border)]">
                            <tr>
                                ...
                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Current Grade</th>
                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Final Grade</th>
                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Evaluation</th>
                ...
                <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-gray-200 dark:border-[var(--card-border)] shadow-sm p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">No students enrolled</h3>
                    <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">
                        {debouncedSearch ? 'No students match your search.' : 'This course has no enrolled students yet.'}
                    </p>
                    {!debouncedSearch && (
                <Link href={`/gradebook/create?courseId=${courseId}`}
                                    ...
                                    <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{getStudentName(e.student as any)}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3"><StatusBadge status={e.status} /></td>
                                        <td className="px-4 py-3"><GradeDisplay grade={e.currentGrade} /></td>
                                        <td className="px-4 py-3"><GradeDisplay grade={e.finalGrade} /></td>
                                        <td className="px-4 py-3"><EvalBadge eval={e.finalEvaluation} /></td>
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Link href={`/gradebook/${courseId}/${e.id}/edit`}
                                                    className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Edit Grades">
                                                ...
                                                <button onClick={() => setDeleteTarget(e)}
                                                    ...
                                                    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div className="bg-white dark:bg-[var(--card-background)] rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6" onClick={e => e.stopPropagation()}>
                        <div className="text-center">
                            <div className="h-12 w-12 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                                <svg className="h-6 w-6 text-red-600 dark:text-red-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                            </div>
                            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">Remove Enrollment</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                                Remove <span className="font-semibold text-gray-700 dark:text-gray-300">{getStudentName(deleteTarget.student as any)}</span> from this course? This action cannot be undone.
                            </p>
            ...
            {/* Detail Slide-Over */}
            {detailEnrollment && (
                <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setDetailEnrollment(null)}>
                    ...
                    <div className="relative w-full max-w-lg bg-white dark:bg-[var(--card-background)] shadow-2xl h-full overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="sticky top-0 bg-white dark:bg-[var(--card-background)] border-b border-gray-200 dark:border-[var(--card-border)] px-6 py-4 flex items-center justify-between z-10">
                            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 truncate pr-4">{getStudentName(detailEnrollment.student as any)}</h2>
                            <button onClick={() => setDetailEnrollment(null)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500 shrink-0">
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                        <div className="p-6 space-y-6">
                                ...
                                <>
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            ...
                                            <div className="mt-1"><StatusBadge status={detailEnrollment.status} /></div>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Type</span>
                                            <p className="font-medium text-gray-900 dark:text-gray-100 mt-1 capitalize">{detailEnrollment.enrollmentType}</p>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Current Grade</span>
                                            <div className="mt-1"><GradeDisplay grade={detailEnrollment.currentGrade} /></div>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Final Grade</span>
                                            <div className="mt-1"><GradeDisplay grade={detailEnrollment.finalGrade} /></div>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Evaluation</span>
                                            <div className="mt-1"><EvalBadge eval={detailEnrollment.finalEvaluation} /></div>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Progress</span>
                                            <p className="font-medium text-gray-900 dark:text-gray-100 mt-1">{detailEnrollment.progressPercentage ?? 0}%</p>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Enrolled</span>
                                            <p className="font-medium text-gray-900 dark:text-gray-100 mt-1">{detailEnrollment.enrolledAt ? new Date(detailEnrollment.enrolledAt).toLocaleDateString() : '-'}</p>
                                        </div>
                                        <div>
                                            <span className="text-gray-500 dark:text-gray-400">Completed</span>
                                            <p className="font-medium text-gray-900 dark:text-gray-100 mt-1">{detailEnrollment.completedAt ? new Date(detailEnrollment.completedAt).toLocaleDateString() : '-'}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-[var(--card-border)]">
                                        <Link href={`/gradebook/${courseId}/${detailEnrollment.id}/edit`}
