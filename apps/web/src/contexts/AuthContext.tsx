  ...
  const initializeAuth = useCallback(async () => {
    ...
    try {
      ...
      if (initialUser && initialToken) {
        try {
          localStorage.setItem('grandline_auth_user_trainee', JSON.stringify(initialUser));
          localStorage.setItem('grandline_auth_token_trainee', initialToken);
        } catch { void 0; }
      ...
      try {
        ...
        cachedToken = localStorage.getItem('grandline_auth_token_trainee');
        if (cached) {
      ...
      // Update local storage token just in case other legacy code needs it
      if (token) {
        localStorage.setItem('grandline_auth_token_trainee', token);
      } else {
