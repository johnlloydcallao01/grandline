          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {!isLoading && !isSelectionMode && unreadCount > 0 && (
              <button
                ...
                className="px-3 py-1.5 bg-[#201a7c] text-white rounded-lg text-sm font-medium hover:bg-[#1a1569] dark:bg-[#3028a3] dark:hover:bg-[#3b32c4] transition-colors"
              >
                Mark all read
              </button>
            )}
            {!isLoading && !isSelectionMode && (
              <button
                ...
                className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors flex items-center space-x-1"
          ...
          {isLoading ? (
            ...
            <div className="flex items-center space-x-4 animate-pulse">
              <div className="flex space-x-2 pb-2 flex-1">
                <div className="h-9 bg-gray-200 dark:bg-gray-800 rounded-lg w-16" />
                <div className="h-9 bg-gray-200 dark:bg-gray-800 rounded-lg w-20" />
                <div className="h-9 bg-gray-200 dark:bg-gray-800 rounded-lg w-24" />
              </div>
            </div>
          ) : (
              ...
              {isSelectionMode && (
                <label className="flex items-center space-x-2 cursor-pointer select-none">
                  <input
                    ...
                    className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-[#201a7c] focus:ring-[#201a7c] cursor-pointer"
                  />
                ...
                {filters.map((type) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex items-center space-x-2 ${selectedFilter === type.id
                      ? 'bg-[#201a7c] text-white dark:bg-[#3028a3]'
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="bg-[var(--card-background)] rounded-lg border border-[var(--card-border)] p-4 animate-pulse">
                <div className="flex items-start space-x-3">
                  {isSelectionMode && <div className="mt-2 w-4 h-4 rounded bg-gray-200 dark:bg-gray-800 flex-shrink-0" />}
          ...
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4 border border-[var(--card-border)]">
              <i className="fa fa-bell-slash text-gray-400 dark:text-gray-500 text-2xl" />
            </div>
          ...
          <div className="space-y-3">
            {filteredNotifications.map((notification) => (
              <div
                key={notification.id}
                className={`bg-[var(--card-background)] rounded-lg border border-[var(--card-border)] overflow-hidden transition-all duration-200 ${!notification.read ? 'border-l-4 border-l-[#201a7c] dark:border-l-[#3028a3] shadow-sm' : 'hover:shadow-sm'
                  }`}
              >
                <div className="p-4">
                  <div className="flex items-start space-x-3">
                    ...
                    {isSelectionMode && (
                      <input
                        ...
                        className="mt-2 w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-[#201a7c] focus:ring-[#201a7c] cursor-pointer flex-shrink-0"
                      />
                    )}
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${notification.iconBg.replace('bg-', 'bg-').replace('/10', '/30')}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h3
                            className={`text-sm font-semibold ${!notification.read ? 'text-gray-900 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300'
                              }`}
                          >
                            {notification.title}
                          </h3>
                          <p
                            className={`text-sm mt-1 ${!notification.read ? 'text-gray-700 dark:text-gray-300' : 'text-gray-600 dark:text-gray-400'
                              }`}
                          >
                        ...
                        <div className="flex items-center space-x-2 ml-4">
                          {!notification.read ? (
                            <button
                              ...
                              className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center hover:bg-blue-200 dark:hover:bg-blue-800/40 transition-colors"
                              title="Mark as read"
                            >
                              <i className="fa fa-check text-blue-600 dark:text-blue-400 text-xs" />
                            </button>
                          ) : (
                            <button
                            ...
                            >
                              <i className="fa fa-envelope text-gray-600 dark:text-gray-400 text-xs" />
                            </button>
                          )}
                          <button
                            onClick={() => deleteNotification(notification.id)}
                            className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors group"
                            title="Delete notification"
                          >
                            <i className="fa fa-trash text-gray-400 dark:text-gray-500 group-hover:text-red-500 dark:group-hover:text-red-400 text-xs" />
                          </button>
                        </div>
                      </div>
