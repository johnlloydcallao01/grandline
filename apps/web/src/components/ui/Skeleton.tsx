...
export const CardSkeleton: React.FC<{ className?: string }> = ({ 
  ...
  return (
    <div className={`bg-[var(--card-background)] rounded-lg border border-[var(--card-border)] ${className}`}>
      {/* Header skeleton */}
  ...
  return (
    <tr>
      {Array.from({ length: columns }).map((_, index) => (
        <td key={index} className="px-6 py-4 border-b border-[var(--card-border)]">
          <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div>
        </td>
      ))}
    </tr>
...
export const PageHeaderSkeleton: React.FC = () => {
  return (
    <div className="mb-8">
      <div className="h-8 bg-gray-200 dark:bg-gray-700 rounded w-1/4 animate-pulse mb-2"></div>
