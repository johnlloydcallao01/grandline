  ...
  return (
    <div className="w-full min-h-screen bg-[var(--background)]">
        ...
        <div className="w-full px-[10px] py-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage and access your maritime training content</p>
            </div>
            ...
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-[var(--card-background)] rounded-xl border border-[var(--card-border)] overflow-hidden animate-pulse">
                ...
                <div className="p-5">
                  ...
                  <div className="h-5 bg-gray-200 dark:bg-gray-800 rounded w-3/4 mb-4 mt-2"></div>
                  <div className="flex items-center gap-2 mb-4">
                    <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-800"></div>
                    <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded w-1/2"></div>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-2 mb-4"></div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredEnrollments.map((enrollment) => {
                ...
                return (
                  <div key={enrollment.id} className="bg-[var(--card-background)] rounded-xl border border-[var(--card-border)] overflow-hidden hover:shadow-md transition-shadow group">
                    {/* Course Image */}
      ...
      {mounted && isEnrollModalOpen && createPortal(
        <div className="fixed inset-0 z-[99999] bg-white dark:bg-gray-900 md:bg-black/50 md:dark:bg-black/70 flex flex-col md:items-start md:justify-center md:p-6 md:pt-12 overflow-hidden" style={{ margin: 0 }}>
          <div className="bg-[var(--background)] w-full h-full flex flex-col md:h-auto md:max-h-[85vh] md:rounded-xl md:max-w-4xl md:shadow-2xl overflow-hidden md:mx-auto border border-transparent md:border-[var(--card-border)]">
            <div className="p-3 md:px-6 md:py-4 flex items-center gap-2 md:gap-4 border-b border-[var(--card-border)] bg-[var(--card-background)] shrink-0">
              {/* Mobile Back Button */}
              <button onClick={() => setIsEnrollModalOpen(false)} className="md:hidden w-8 h-8 rounded-full bg-[var(--card-background)] border border-[var(--card-border)] shadow-md flex items-center justify-center shrink-0 text-gray-700 dark:text-gray-300">
                <i className="fa fa-arrow-left"></i>
              </button>

              {/* Desktop Title */}
              <h2 className="hidden md:block text-xl font-bold text-gray-900 dark:text-gray-100">Available Courses</h2>
              ...
              <button onClick={() => setIsEnrollModalOpen(false)} className="hidden md:block text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors p-1 shrink-0 ml-2">
              ...
              {loadingCourses ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="border border-[var(--card-border)] bg-[var(--card-background)] rounded-xl overflow-hidden flex flex-col animate-pulse">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {availableCourses.filter(c => c.title?.toLowerCase().includes(modalSearchQuery.toLowerCase())).map(course => {
                    ...
                    return (
                      <div key={course.id} className="border border-[var(--card-border)] bg-[var(--card-background)] rounded-xl overflow-hidden flex flex-col">
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                          <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-1 line-clamp-2">{course.title}</h3>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 capitalize">{course.difficultyLevel || 'Standard'}</p>
                          <div className="mt-auto">
                            {isEnrolled ? (
                              <button disabled className="w-full py-2 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-lg text-sm font-medium cursor-not-allowed">
                                Already Enrolled
                              </button>
                            ) : (
                              <button
