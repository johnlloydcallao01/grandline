...
export default function AskInstructorThreadPage() {
    ...
    if (loadingMessages) {
        return (
            <div className="w-full px-[10px] py-6 h-[calc(100vh-100px)] flex flex-col max-w-5xl mx-auto animate-pulse">
                {/* Back Link Skeleton */}
                <div className="w-32 h-5 bg-gray-200 dark:bg-gray-800 rounded mb-4 shrink-0" />
                ...
                <div className="bg-[var(--card-background)] rounded-t-xl border-t border-x border-[var(--card-border)] p-4 shadow-sm shrink-0 flex items-center justify-between">
                    <div>
                        <div className="h-6 bg-gray-200 dark:bg-gray-800 rounded w-48 mb-2" />
                        <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded w-32" />
                    </div>
                </div>

                {/* Messages Area Skeleton */}
                <div className="flex-1 bg-gray-50 dark:bg-[var(--background)] border-x border-[var(--card-border)] overflow-y-auto p-4 md:p-6 space-y-6">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className={`flex gap-3 ${i % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[70%] flex flex-col ${i % 2 === 0 ? 'items-end' : 'items-start'}`}>
                                ...
                                <div className={`p-4 rounded-2xl w-48 md:w-64 h-16 ${
                                    i % 2 !== 0 
                                    ? 'bg-[var(--card-background)] border border-[var(--card-border)] rounded-tl-sm' 
                                    : 'bg-[#201a7c]/20 dark:bg-[#3028a3]/40 rounded-tr-sm'
                                }`} />
                            </div>
                            {i % 2 === 0 && (
                                <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-800 shrink-0" />
                ...
                <div className="bg-[var(--card-background)] rounded-b-xl border border-[var(--card-border)] p-4 shadow-sm shrink-0">
                    <div className="flex gap-3 items-end">
                        <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-xl h-[50px]" />
    ...
    return (
        <div className="w-full px-[10px] py-6 h-[calc(100vh-100px)] flex flex-col max-w-5xl mx-auto">
            ...
            <div className="bg-[var(--card-background)] rounded-t-xl border-t border-x border-[var(--card-border)] p-4 shadow-sm shrink-0 flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{activeTopic.subject}</h1>
                    <div className="flex items-center gap-2 text-sm mt-1">
                        <span className="text-gray-500 dark:text-gray-400">With: <span className="font-medium text-gray-900 dark:text-gray-200">{activeTopic.instructor}</span></span>
                        <span className="text-gray-300 dark:text-gray-600">•</span>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${activeTopic.status === 'answered' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' :
                            activeTopic.status === 'archived' ? 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400'
                            }`}>
                            {activeTopic.status}
                        </span>
                    </div>
