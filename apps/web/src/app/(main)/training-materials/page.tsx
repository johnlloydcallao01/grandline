    ...
    <div className="min-h-screen bg-[var(--background)] pb-12">
        ...
        <div className="w-full px-[10px]">
          ...
          <div className="flex space-x-8 overflow-x-auto pb-px">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-6 w-16 bg-gray-200 dark:bg-gray-800 rounded mb-4 animate-pulse"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-[10px] py-8">
        <div className="mb-8 max-w-lg animate-pulse">
          <div className="h-12 bg-gray-200 dark:bg-gray-800 rounded-md w-full"></div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-[var(--card-background)] rounded-xl shadow-sm border border-[var(--card-border)] flex flex-col animate-pulse">
              <div className="h-32 bg-gray-200 dark:bg-gray-800 rounded-t-xl"></div>
