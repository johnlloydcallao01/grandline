...
export default function TrainingMaterialsClient({ initialMaterials }: TrainingMaterialsClientProps) {
  ...
  const renderMaterialCard = (item: FrontendMaterial) => (
    <Link 
      ...
      className="bg-[var(--card-background)] rounded-xl shadow-sm border border-[var(--card-border)] hover:shadow-md transition-shadow duration-200 flex flex-col group cursor-pointer"
    >
      <div className={`h-32 ${item.image} rounded-t-xl flex items-center justify-center relative group-hover:opacity-90 transition-opacity`}>
        {item.isRequired && (
          <span className="absolute top-3 right-3 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400 text-xs font-medium px-2 py-0.5 rounded border border-red-200 dark:border-red-800/50">
            Required
          </span>
        )}
         {/* Placeholder Icons based on type */}
         {item.type === 'PDF' && (
           <svg className="w-12 h-12 text-gray-600 dark:text-gray-400 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="mt-3 text-base font-semibold text-gray-900 dark:text-gray-100 line-clamp-2" title={item.title}>
              {item.title}
            </h3>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{item.courseName}</p>
            {item.lessonTitle && (
              <p className="mt-0.5 text-xs text-indigo-500 dark:text-indigo-400 font-medium line-clamp-1">Lesson: {item.lessonTitle}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <span className="text-xs text-gray-400 dark:text-gray-500">Added {item.date}</span>
          <span className="flex items-center text-sm font-medium text-blue-600 dark:text-blue-400 group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">
    ...
    <div className="min-h-screen bg-[var(--background)] pb-12">
      ...
      <div className="w-full px-[10px] py-8">
        ...
        <div className="mb-8 max-w-lg">
          <div className="relative rounded-md shadow-sm">
            ...
            <input
              ...
              className="focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 sm:text-sm border-gray-300 dark:border-gray-700 bg-[var(--background)] text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 rounded-md py-3"
        ...
        {(!initialMaterials || initialMaterials.length === 0) && (
          <div className="text-center py-16 bg-[var(--card-background)] rounded-xl border border-[var(--card-border)]">
            <svg className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No materials found</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">You are not enrolled in any courses with available materials yet.</p>
          </div>
        )}
        ...
        {courseMaterials.length > 0 && (
          <div className="mb-12">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-6">Course Materials</h2>
        ...
        {lessonMaterials.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-6">Lesson Materials</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
