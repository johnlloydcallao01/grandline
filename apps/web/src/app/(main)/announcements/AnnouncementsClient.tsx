...
export default function AnnouncementsClient({ initialAnnouncements }: AnnouncementsClientProps) {
  ...
  const renderAnnouncement = (announcement: Announcement) => {
    ...
    return (
      <div key={announcement.id} className={`bg-[var(--card-background)] rounded-xl shadow-sm border ${announcement.pinned ? 'border-blue-300 dark:border-blue-800 ring-1 ring-blue-100 dark:ring-blue-900/50' : 'border-[var(--card-border)]'} overflow-hidden mb-6`}>
        <div className={`px-6 py-4 border-b ${announcement.pinned ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800' : 'bg-gray-50 dark:bg-gray-800/50 border-[var(--card-border)]'} flex flex-col sm:flex-row sm:items-center justify-between gap-4`}>
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <div className="relative w-full sm:w-64 md:w-80">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg leading-5 bg-[var(--background)] text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:placeholder-gray-400 dark:focus:placeholder-gray-500 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition duration-150 ease-in-out"
                />
              </div>
              <select
                value={filterCourse}
                onChange={(e) => setFilterCourse(e.target.value)}
                className="block w-full sm:w-48 pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-700 bg-[var(--background)] text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-lg"
              >
      ...
      <div className="w-full px-[10px] py-8">
        {initialAnnouncements.length === 0 ? (
          <div className="text-center py-16 bg-[var(--card-background)] rounded-xl border border-[var(--card-border)]">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">You don't have any announcements at the moment.</p>
          </div>
          ...
          <div className="w-full">
            {pinnedAnnouncements.length > 0 && (
              <div className="mb-8">
                <h2 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Pinned</h2>
            ...
            {regularAnnouncements.length > 0 && (
              <div>
                {pinnedAnnouncements.length > 0 && (
                  <h2 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Recent</h2>
                )}
