    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[var(--background)]">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Please Sign In</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-6">You need to be logged in to access the support center.</p>
          <Link href="/signin" className="inline-block bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 dark:hover:bg-blue-500 transition-colors">
            Sign In
          </Link>
        </div>
      </div>
          ...
          <div className="py-6 flex justify-between items-center">
            ...
            {view === 'list' && (
              <button
                ...
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 dark:hover:bg-blue-500 transition-colors flex items-center gap-2"
              >
      ...
      <div className="w-full px-[10px] py-8">
              ...
              {loading && tickets.length === 0 ? (
                Array.from({ length: 2 }).map((_, idx) => (
                  <div key={idx} className="bg-[var(--card-background)] p-6 rounded-xl shadow-sm border border-[var(--card-border)] animate-pulse">
                    <div className="h-5 bg-gray-200 dark:bg-gray-800 rounded w-24 mb-3"></div>
                    <div className="h-8 bg-gray-200 dark:bg-gray-800 rounded w-16 mb-2"></div>
                    <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded w-32 mt-1"></div>
                  </div>
                ))
              ) : (
                <>
                  <div className="bg-[var(--card-background)] p-6 rounded-xl shadow-sm border border-[var(--card-border)]">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">My Tickets</h3>
                    <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{tickets.length}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Total tickets submitted</p>
                  </div>
                  <div className="bg-[var(--card-background)] p-6 rounded-xl shadow-sm border border-[var(--card-border)]">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Open Issues</h3>
                    <p className="text-3xl font-bold text-yellow-600 dark:text-yellow-400">
                      {tickets.filter(t => t.status === 'open' || t.status === 'in_progress').length}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Tickets currently active</p>
                  </div>
                </>
              )}
            </div>
              ...
              {loading && tickets.length === 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-[var(--card-border)]">
                      <tr>
                      ...
                      {Array.from({ length: 3 }).map((_, idx) => (
                        <tr key={idx} className="animate-pulse">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right">
                            <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded w-10 ml-auto"></div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="mt-2 text-gray-500 dark:text-gray-400 mb-6">You haven't submitted any support tickets yet.</p>
                  <button
                    onClick={() => setView('create')}
                    className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:hover:bg-blue-500"
                  >
        ...
        {view === 'create' && (
          <div className="max-w-3xl mx-auto">
            <div className="bg-[var(--card-background)] rounded-xl shadow-sm border border-[var(--card-border)] overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--card-border)] bg-gray-50 dark:bg-gray-800/50">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Create New Ticket</h2>
              </div>
              <form onSubmit={handleCreateSubmit} className="p-6 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    id="subject"
                    required
                    className="w-full rounded-lg border-gray-300 dark:border-gray-700 bg-[var(--background)] text-gray-900 dark:text-gray-100 border p-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500 dark:placeholder-gray-400"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      id="category"
                      required
                      className="w-full rounded-lg border-gray-300 dark:border-gray-700 bg-[var(--background)] text-gray-900 dark:text-gray-100 border p-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-[var(--card-border)]">
                  <button
                    type="button"
                    onClick={() => setView('list')}
                    className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-[var(--card-background)] hover:bg-gray-50 dark:hover:bg-gray-800"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:hover:bg-blue-500 disabled:opacity-50"
                  >
