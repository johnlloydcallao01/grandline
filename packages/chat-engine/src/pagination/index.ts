...
export function decodeCursor(cursorString: string): MessageCursor | null {
  try {
    const json = typeof Buffer !== 'undefined'
      ? Buffer.from(cursorString, 'base64').toString('utf-8')
      : atob(cursorString)
    const parsed = JSON.parse(json) as MessageCursor
...
export function buildPaginationQuery(options: PaginationOptions): PaginationQuery {
...
export function getCursorFromMessage(message: ChatMessage): MessageCursor {
  return {
...
export function getNextCursor(messages: ChatMessage[], limit: number): string | null {
...
export function getPrevCursor(messages: ChatMessage[]): string | null {
...
export function buildPaginatedResult<T>(
  ...
  getCursor: (item: T) => MessageCursor
): PaginatedResult<T> {
...
export function normalizePaginationOptions(
  ...
  const limit = Math.min(
    Math.max(options.limit ?? DEFAULT_PAGE_SIZE, MIN_PAGE_SIZE),
    MAX_PAGE_SIZE
  )
...
export function appendMessages(
  ...
  result: PaginatedResult<ChatMessage>
): PaginationState {
  return {
    ...state,
