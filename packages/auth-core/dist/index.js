...
__export(index_exports, {
  AuthPresets: () => AuthPresets,
  AuthValidator: () => AuthValidator,
  DEFAULT_API_URL: () => DEFAULT_API_URL,
  PayloadCMSAuth: () => PayloadCMSAuth,
  ValidationUtils: () => ValidationUtils,
  createAdminAuthConfig: () => createAdminAuthConfig,
  createAdminAuthMiddleware: () => createAdminAuthMiddleware,
  createAuthMiddleware: () => createAuthMiddleware,
  createInstructorAuthConfig: () => createInstructorAuthConfig,
  createInstructorAuthMiddleware: () => createInstructorAuthMiddleware,
  createMultiRoleAuthConfig: () => createMultiRoleAuthConfig,
  createMultiRoleAuthMiddleware: () => createMultiRoleAuthMiddleware,
  createTraineeAuthConfig: () => createTraineeAuthConfig,
  createTraineeAuthMiddleware: () => createTraineeAuthMiddleware,
  defaultSecurityConfig: () => defaultSecurityConfig,
  getFullName: () => getFullName,
  getUserInitials: () => getUserInitials,
  highSecurityConfig: () => highSecurityConfig,
  standardSecurityConfig: () => standardSecurityConfig,
  useAuth: () => useAuth
});
...
function useAuth(allowedRole) {
  ...
  const [securityAlert, setSecurityAlert] = (0, import_react.useState)(null);
...
function createAuthMiddleware(config) {
  return async function authMiddleware(request) {
    ...
    if (!payloadToken) {
      ...
      return import_server.NextResponse.redirect(new URL(config.loginPath, request.url));
    }
    try {
      ...
      if (response.ok) {
        ...
        if (user && !config.allowedRoles.includes(user.role)) {
          ...
          const response2 = import_server.NextResponse.redirect(new URL(config.loginPath, request.url));
